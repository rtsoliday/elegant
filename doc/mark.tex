...
\item If moments calculations are being performed (via {\tt moments\_output} with  {\tt output\_at\_each\_step=1}), then the quantities 
 {\tt s{\em i}{\em j}m}, $1 \leq i\leq j \leq 6$, giving the 21 unique elements of the sigma matrix, are available, as are
 {\tt s{\em i}{\em j}betam}, $1 \leq i \leq j 4$, giving the 6  unique elements with the dispersive terms removed.
 The quantities {\tt c{\em i}m}, $1\leq i \leq 6$,
 are also created, giving the 6 centroids from the moments computation. 
 The projected betatron emittances are available using {\tt exbeta} and {\tt eybeta}.
 In addition, the emittances of the three modes are available using 
 {\tt e{\em i}m}, $1\leq i \leq 3$, without any prefix.
 The {\tt m} on the end of the symbols is to distinguish them from the moments computed from tracking.
