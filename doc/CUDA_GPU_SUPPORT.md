...
| `ELEGANT_GPU_MIN_EXACT_DRIFT_PARTICLES=N` | Exact-drift threshold.  Exact drift is correct but disabled by default unless `ELEGANT_GPU_ENABLE_EXACT_DRIFT=1` is set. |
...
| `ELEGANT_GPU_MIN_SCMULT_PARTICLES=N` | `SCMULT` threshold for the opt-in Phase 7 slice. |
| `ELEGANT_GPU_ENABLE_EXACT_DRIFT=1` | Enable exact-drift CUDA experiments. |
| `ELEGANT_GPU_ENABLE_APERTURE_COMPACTION=1` | Enable simple `MAXAMP` CUDA loss compaction experiments.  This exact-order path remains opt-in; Phase 15 reduces normal-build readback to only the lost tail rows, while `GPU_VERIFY` keeps full CPU-shadow synchronization. |
| `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=1` | Enable the Phase 15 stable prefix-sum aperture loss-compaction prototype.  It preserves the verified simple rectangular/elliptical `MAXAMP`, non-inverted `RCOL` including zero-length and nonzero-length open-sided `RCOL`, nonzero-length open-sided `RCOL` global loss-coordinate output, non-inverted even-exponent `ECOL` including open-sided and mixed-`YEXPONENT` cases with global loss-coordinate output, ideal `SCRAPER` including one-sided and two-sided global loss-coordinate output, interpolated rectangular `aperture_data`, and narrow identity-`RFCA` `removeInvalidParticles` loss bookkeeping, including accepted counts and loss values by `particleID` where supported.  It promotes the compacted scratch coordinate buffer to resident device storage instead of doing a full device-to-device copy and keeps `accepted` compaction on the device by default.  The path remains opt-in because the current production-style win is focused and small/watch-heavy cases are still startup and synchronization dominated. |
| `ELEGANT_GPU_ENABLE_APERTURE_ACCEPTED_DEVICE=0|1` | Controls device-side `accepted` array compaction when `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=1` or `ELEGANT_GPU_ENABLE_MAGNET_LOSS_COMPACTION=1`.  The default is enabled with those stable compaction paths; set it to `0` to force the older prefix readback and host-side accepted partition while debugging. |
| `ELEGANT_GPU_ENABLE_MAGNET_LOSS_COMPACTION=1` | Enable the stable prefix-sum magnet loss-compaction prototype for supported multipole, non-CSR `CSBEND`, and action-8 `KICKMAP`/`UKICKMAP` kernels when no losses file or global loss-coordinate bookkeeping is active.  Accepted-array partitioning is supported through the same stable prefix map.  Detected magnet and map losses fall back to CPU by default, and `.los` or global-loss-coordinate output still forces CPU loss-row handling; this opt-in path is for no-loss-output survivor-residency testing. |
| `ELEGANT_GPU_ENABLE_CSR_HISTOGRAM=1` | Enable experimental fixed-bin CSR histogram fill.  The Phase 14 path packs only the longitudinal coordinate before the histogram host-to-device copy, packs `ctHist` and `ctHistDeriv` into one wake-kernel upload, caches and reuses the CSR denominator table on the host and device when `nBins` and `dct` are unchanged, skips ordinary `T1`/`T2` host returns unless wake output or `GPU_VERIFY` needs them, and verifies non-IGF `CSR_LAST_WAKE` handoff state in `GPU_VERIFY`, including wake-filter-table postprocessing. |
| `ELEGANT_GPU_ENABLE_CSR_KICK=1` | Enable the Phase 14 prototype CUDA CSR particle-kick kernel.  It is off by default because the current host-packed implementation is correct but slower than the default CSR histogram path on the 20,000-particle same-workload gate; it is retained as a stepping stone for a future fully resident CSRCSBEND path. |
| `ELEGANT_GPU_ENABLE_CSR_RESIDENT=1` | Enable the Phase 14 opt-in resident `CSRCSBEND` prototype.  For supported non-IGF, non-radiating, non-backtracking CSRCSBEND sections, it keeps the body slice, coordinate-only device-side ct range reduction, histogram fill, wake calculation, and CSR kick on resident particle coordinates, preserves per-particle resident scratch across CSR wake-array growth, reuses CSRCSBEND body rollback/loss buffers, allows adjacent simple matrix/drift islands to remain GPU-resident when the upcoming CSRCSBEND is resident-eligible, performs checked CUDA simple initial/final transforms including first-order entrance/exit edge focusing when particles are already device-current, keeps no-CSR `CSRDRIFT` sections resident as exact or linearized drifts, avoids full CSRDRIFT final-prep for Stupakov-only drift consumers, keeps those final coordinates GPU-resident when safe, and hands off to the CPU for remaining CSRDRIFT state and unsupported options. |
| `ELEGANT_GPU_ENABLE_SCMULT=1` | Enable the opt-in linear, unsliced, single-bucket `SCMULT` CUDA slice.  The May 9, 2026 action-8 `scRing2_no_watch` refresh matched CPU output at `1e-11` with 8 resident SCMULT kernels and only final deallocation synchronization; focused `phase65_scmult_nonlinear_fallback`, `phase66_scmult_sliced_fallback`, and `phase67_scmult_multibunch_fallback` gates confirm nonlinear, sliced, and multi-bunch SCMULT remain CPU-owned under the same opt-in flag.  Keep this opt-in pending a release decision or second source-family validation. |
...
- Loose rectangular and elliptical aperture predicates for `MAXAMP`, `RCOL`, `ECOL`, and simple one-sided `SCRAPER`; exact lossy bookkeeping falls back to CPU by default.
- Opt-in stable lossy aperture compaction for selected `MAXAMP`, `RCOL` including zero-length and nonzero-length open-sided `RCOL` with nonzero-length open global-loss-coordinate output, open-sided and mixed-`YEXPONENT` simple `ECOL` with global-loss-coordinate output, ideal `SCRAPER` including one-sided and two-sided global-loss-coordinate output, `aperture_data`, and narrow identity-`RFCA` `removeInvalidParticles` cases.
- Deterministic `KQUAD`, `KSEXT`, `KOCT`, `DQCOR`, simple `MULT`, and non-CSR `CSBEND` slices, including simple original-mode `DX`/`DY`/`DZ`/`TILT` misalignments for `KQUAD`, `KSEXT`, `KOCT`, `DQCOR`, and `MULT`, plus simple original-mode `DX`/`DY`/`DZ`/`TILT`/`ETILT` for non-CSR `CSBEND`.  The `MULT` subset is limited to deterministic `ORDER=0..3`, `N_SLICES>0`, no synchrotron radiation, no spin tracking, and no aperture hook.  Detected magnet losses still fall back to CPU by default, with an opt-in no-loss-output stable compaction prototype under `ELEGANT_GPU_ENABLE_MAGNET_LOSS_COMPACTION=1`, including accepted-array partitioning; `.los` and global-loss-coordinate output remain on the CPU loss-row path.
...
- Opt-in linear, unsliced, single-bucket `SCMULT`, including the production-shaped `scRing2_no_watch` action-8 quick refresh.  Nonlinear, sliced single-bunch, and multi-bunch modes are guarded by focused CPU-fallback gates rather than resident CUDA kernels.
