...
\item \verb|filename| --- The (incomplete) name of the SDDS file to which the
  response matrix is written.  The format includes the BPM/skew element names
...
\item \verb|name_pattern|, \verb|type_pattern| --- Space-separated lists of
  name and type patterns (with the usual \verb|*| / \verb|?| wildcards) used
  to select the skew-quadrupole elements that act as correction knobs.  At
  least one of these must be supplied; an element is taken as a knob if it
  matches any \verb|name_pattern| entry (or all if none is given) and any
  \verb|type_pattern| entry (or all if none is given).
\item \verb|item| --- The name of the element parameter to perturb when
  computing the response (e.g., \verb|K1|).  The skew strength is created by
  whatever the lattice element provides; commonly this is a \verb|QUAD| or
  \verb|KQUAD| with \verb|TILT=|$\pi/4$ and \verb|K1| as the knob.
...
The values of \verb|name_pattern|, \verb|type_pattern|, \verb|item|,
\verb|bpm_name_pattern|, and \verb|bpm_type_pattern| are also recorded
implicitly in the saved file through the element names and occurrences,
so a subsequent \verb|correct_coupling| (whether in this run or a later one
via \verb|load_coupling_response_matrix|) does not need to repeat them.
...
\item \verb|name_pattern|, \verb|type_pattern| --- Space-separated lists of
  name and type patterns (with the usual \verb|*| / \verb|?| wildcards) used
  to select the (normal) quadrupole elements that act as correction knobs.  At
  least one of these must be supplied; an element is taken as a knob if it
  matches any \verb|name_pattern| entry (or all if none is given) and any
  \verb|type_pattern| entry (or all if none is given).
\item \verb|bind_name_pattern| --- Optional family-binding specification
  ...
  carries.
\item \verb|item| --- The name of the element parameter to perturb when
  computing the response (e.g., \verb|K1|).
\item \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists
  selecting the BPM observation points.  Default
  \verb|bpm_type_pattern="MONI"| picks all monitor elements.
\item \verb|response_perturbation| --- Finite-difference step in \verb|item|
...
\verb|bpm_name_pattern|, and \verb|bpm_type_pattern| are also recorded
implicitly in the saved file through the element names and occurrences,
so a subsequent \verb|correct_lattice| (whether in this run or a later one
via \verb|load_lattice_response_matrix|) does not need to repeat them.
...
\item function: minimize the vertical dispersion $\eta_y$ at the selected
  ...
  LOCO-style vertical-dispersion channel of real-machine coupling correction.
...
\begin{itemize}
\item \verb|name_pattern|, \verb|type_pattern|, \verb|item|,
  \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists and
  parameter name selecting the skew-quadrupole knobs and BPM observation
  points; same semantics as in \verb|compute_coupling_response_matrix|.
  If \verb|compute_coupling_response_matrix| or
  \verb|load_coupling_response_matrix| has already been issued, the
  preloaded knob/BPM inventory is used and these namelist values are
  ignored.
...
\item \verb|correction_fraction| --- Fraction of the SVD-derived step
  ...
  overshoot; the default 0.5 is a conservative choice for typical
  LOCO-style usage.  Must be in $(0,1]$.
...
\item \verb|adaptive_step| --- If nonzero, the effective
  ...
  RMS $\eta_y$ larger than at the start of the previous iteration.  Same
  pattern as \verb|chromaticity| uses for its correction fraction.
...
\item \verb|strength_limit| --- If $>0$, the per-iteration step is uniformly
  scaled so that no knob's parameter value exceeds \verb|strength_limit| in
  absolute value after the correction is applied.  Setting all knobs would
  otherwise require simultaneously honoring per-knob limits, which would
  distort the SVD direction; uniform scaling preserves the direction at the
  cost of a shorter step.
...
correction run; in that case the pattern/item parameters need not be
repeated on \verb|correct_coupling|.
...
\item function: drive the linear lattice functions $\beta_x$, $\beta_y$,
  ...
  knobs.  This emulates the LOCO-style $\beta$-beat and dispersion channels
  of real-machine optics correction.
\item sequence: must follow \verb|run_setup| and \verb|twiss_output|.  A
  \verb|track| command (or another action command that triggers the steps
  loop) must follow in order for the correction to actually run.
...
\item \verb|name_pattern|, \verb|type_pattern|, \verb|item|,
  \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists and
  parameter name selecting the normal-quadrupole knobs and BPM observation
  points; same semantics as in \verb|compute_lattice_response_matrix|.
  If \verb|compute_lattice_response_matrix| or
  \verb|load_lattice_response_matrix| has already been issued, the
  preloaded knob/BPM inventory is used and these namelist values are
  ignored.
...
\item \verb|adaptive_step| --- If nonzero, the effective
  ...
  Same pattern as \verb|chromaticity| uses for its correction fraction.
\item \verb|response_perturbation| --- Finite-difference step used when
  this command builds the matrix itself (only relevant when
  \verb|use_perturbed_matrix>0| or when no preloaded matrix exists).
\item \verb|svd_threshold| --- Singular values below
...
\item \verb|strength_limit| --- If $>0$, the per-iteration step is uniformly
  scaled so that no knob's parameter value exceeds \verb|strength_limit| in
  absolute value after the correction is applied.  Uniform scaling preserves
  the SVD direction at the cost of a shorter step.
...
the pattern/item parameters need not be repeated on
\verb|correct_lattice|.
...
knob/BPM/parameter selection on \verb|correct_coupling| is not required
(and is ignored if supplied).  If neither is given, \verb|correct_coupling|
builds the matrix itself according to its own \verb|use_perturbed_matrix|
control.
...
knob/BPM/parameter selection on \verb|correct_lattice| is not required
(and is ignored if supplied).  If neither is given, \verb|correct_lattice|
builds the matrix itself according to its own \verb|use_perturbed_matrix|
control.
