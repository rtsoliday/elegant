...
\item function: compute and save the response matrix
  $R_{ij} = \partial\eta_y(\mathrm{BPM}_i)/\partial K_{1,j}$ used by
...
\item \verb|cross_filename| --- The (incomplete) name of a {\em second} SDDS
  ...
  everything without the user re-supplying it.  Default \verb|NULL| means
  no cross-plane matrix is built or saved -- existing $\eta_y$-only files
  are produced byte-identical to earlier versions.
...
\item \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists
  selecting the BPM observation points.  Default
  \verb|bpm_type_pattern="MONI"| picks all monitor elements.
...
\item type: setup command.
\item function: compute and save the response matrix
  $R_{ij,k} = \partial o_k(\mathrm{BPM}_i)/\partial K_{1,j}$ used by
  \verb|correct_lattice|, where $o_k \in \{\beta_x, \beta_y, \eta_x\}$
  enumerates the three observables tracked per BPM.  The saved matrix
...
\begin{itemize}
\item \verb|filename| --- The (incomplete) name of the SDDS file to which the
  response matrix is written.  The format includes the BPM/knob element names
  ...
  knob/BPM inventory from this single file.  Suggested value: ``\%s.lrm''.
\item \verb|correction_elements| --- Whitespace/comma-separated list of
  wildcard patterns (one per family) selecting the elements that act as
...
\item type: setup command.  Execution is deferred to the simulation steps
  ...
  and tune correction.
\item function: minimize the vertical dispersion $\eta_y$ at the selected
  BPMs by adjusting selected skew-quadrupole knobs.  This emulates the
  LOCO-style \cite{Safranek-LOCO} vertical-dispersion channel of real-machine coupling correction.
...
\item \verb|cross_response_weight| --- Weight applied to every cross-plane
  row of the residual vector.  Default 0.0 disables the channel; any
  positive value enables it and turns on the cross-plane steering machinery
  described below.  When both \verb|etay_weight| and
...
\paragraph{Cross-plane channel.}  When \verb|cross_response_weight > 0|,
the SVD residual gains an additional block of rows: the change in the
cross-plane closed-orbit centroid at each selected BPM per unit kick
applied to each selected steering corrector.  Specifically, for each H
probe corrector, the vertical centroid at every \verb|cross_y_bpm_*|
monitor enters one row; for each V probe corrector, the horizontal
centroid at every \verb|cross_x_bpm_*| monitor enters one row.  These
rows are zero in an uncoupled lattice, and grow with coupling, so
driving them toward zero through skew-quad knobs minimizes the
machine's coupling.  The cross-plane reader works by perturbing each
probe corrector by \verb|cross_steering_kick|, finding the closed orbit
of the perturbed lattice, subtracting the unperturbed orbit, and
dividing by the kick; the implementation is completely independent of
the orbit-correction setup (\verb|&correct|, \verb|&steering_element|).
The cost is one closed-orbit calculation per probe corrector per skew-knob
perturbation, so users should pick a modest subset of correctors.

For a typical correction workflow the user issues a
\verb|twiss_output|, then \verb|correct_coupling|, then a
\verb|twiss_output| with \verb|output_at_each_step=1| (to capture the
post-correction state), then a \verb|bunched_beam|/\verb|track| pair to
trigger the steps loop.  When a precomputed matrix is to be reused across
many runs, the matrix can be produced once by
\verb|compute_coupling_response_matrix| (perhaps in an upstream design
run) and loaded by \verb|load_coupling_response_matrix| in each
correction run.  In that case \verb|correction_elements| and \verb|items|
need not be repeated on \verb|correct_coupling| -- they are inherited
from the matrix file's \verb|CorrectionElements| and \verb|CorrectionItems|
parameters.  This lets the user specify only \verb|lower_limits| and
\verb|upper_limits| aligned with the inherited family count without
having to restate the patterns; if patterns or items {\em are} restated
they must agree with those recorded in the matrix file, otherwise the
run aborts with a clear diagnostic.
...
\item function: drive the linear lattice functions $\beta_x$, $\beta_y$,
  and $\eta_x$ at the selected BPMs toward design values loaded from a
...
\item \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists for
  the observation BPMs; same semantics as in
  \verb|compute_lattice_response_matrix|.  Ignored when a preloaded matrix
  carries its own BPM list.
...
For a typical correction workflow the user produces a reference twiss
file from the design lattice (a \verb|run_setup|/\verb|twiss_output|
pair, with no errors loaded), then in the correction run issues
\verb|twiss_output|, \verb|correct_lattice| (pointing
\verb|reference_file| at the design twiss), a \verb|twiss_output| with
\verb|output_at_each_step=1| (to capture the post-correction state), and
finally a \verb|bunched_beam|/\verb|track| pair to trigger the steps
loop.  When a precomputed matrix is to be reused across many runs, the
matrix can be produced once by \verb|compute_lattice_response_matrix|
(perhaps in an upstream design run) and loaded by
\verb|load_lattice_response_matrix| in each correction run.  In that case
\verb|correction_elements| and \verb|items| need not be repeated on
\verb|correct_lattice| -- they are inherited from the matrix file's
\verb|CorrectionElements| and \verb|CorrectionItems| parameters.  This
lets the user specify only \verb|lower_limits| and \verb|upper_limits|
aligned with the inherited family count without having to restate the
patterns; if patterns or items {\em are} restated they must agree with
those recorded in the matrix file, otherwise the run aborts with a clear
diagnostic.
...
\bibitem{Safranek-LOCO} J. Safranek, ``Experimental Determination of Storage Ring Optics Using Orbit Response Measurements,''
  NIM A, vol. 388, page 27, 1997.

\bibitem{PhysRevAccelBeams.27.124401} H. Fu {\em et al.}, ``New analytical formula on the Touschek lifetime of the
  polarized electron beam in electron storage rings,'' Phys. Rev. Accel. Beams, vol. 27, 124401, 2024.

\bibitem{SokolovTernov1964} A. A. Sokolov and I. M. Ternov, ``On polarization and spin effects in
  the theory of synchrotron radiation,'' Sov. Phys. Doklady, vol. 8, page 1203, 1964.

\bibitem{Mane2005} S. R. Mane, Yu. M. Shatunov, and K. Yokoya, ``Spin-polarized charged
  particle beams in high-energy accelerators,'' Rep. Prog. Phys., vol. 68, page 1997, 2005.
