...
\author{Program Version 2026.5\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 2026.5}

Here is a summary of what's changed since release 2026.4.  Historical change logs are collected in Section
\ref{sect:changeLog}.

\subsection{Bug Fixes for Elements}

\begin{itemize}
\item \verb|CBSCAT| (Compton backscattering) automatically uses the exact-recoil model when the Klein-Nishina cross-section is chosen,
   to correct an inconsistency in the conventions for the reference angle (photon vs electron).
\end{itemize}
...
\begin{itemize}
\item The \verb|closed_orbit|, \verb|correct|, and \verb|correction_matrix_output| support a new value for the \verb|fixed_length| and
  \verb|fixed_length_matrix| parameters. In particular, when a value of \verb|2| is given, a six-dimensional closed orbit method is used.
  Previously, a similar method was available using a value of \verb|1|, but this used a four-dimensional orbit method inside a loop that
  determined the energy offset. The new method is faster.
\end{itemize}
...
Starting in Version 2025.4, AI coding assistants, primarily Claude and ChatGPT, have been used for addition of new
features and generation of documentation.
...
This includes the {\tt FRFMODE}, {\tt FTRFMODE}, {\tt LRWAKE}, {\tt RFMODE}, {\tt WAKE}, {\tt TRFMODE}, {\tt TRWAKE}, {\tt CWAKE}, {\tt ZLONGIT}, {\tt ZTRANSVERSE}, and {\tt IMPEDANCE} elements, as well as space charge effects via the \verb|insert_sceffects| command and ion effects via the
...
\item \verb|name| --- A possibly-wildcard-containing string giving the names of the
        ...
        May alternatively name a knob defined via \verb|&load_knobs| (see
        \ref{subsec:loadknobs}); in that case \verb|item|, \verb|type|,
        \verb|exclude|, \verb|start_occurence|/\verb|end_occurence|,
        \verb|s_start|/\verb|s_end|, \verb|after|/\verb|before|, and
        \verb|string_value| are ignored, \verb|multiplicative| is rejected,
        and \verb|value| (combined with \verb|differential| as usual) sets
        the knob's scalar.  The new knob value cascades to every linked
        element parameter as $\Delta\cdot\textrm{Factor}$.
\item \verb|item| --- The name of the parameter to alter.  Required for
        element targets; ignored when \verb|name| is a knob.
...
\item \verb|save_initial_coordinates| --- A flag that, if non-zero, results
...
\item \verb|reset_correctors_each_step| --- If nonzero (the default), the chromaticity
  sextupole strengths are snapshotted on the first \verb|vary_beamline| tick after
  \verb|run_setup| and reasserted at the start of every subsequent step, before
  errors and variations are added.  Set to 0 to inherit the previous step's converged
  sextupole strengths.  See the description under \verb|&correct| for the wider behavior.
This parameter was added in version 2026.3 and changed the default behavior for multi-step simulations.
\end{itemize}
...
\item \verb|fixed_length| --- Selects the closed-orbit mode:
\begin{itemize}
\item \verb|0| (default) --- 4-D transverse closed orbit at fixed momentum offset. The rf
  frequency implicitly varies so that the beam energy is held fixed, even when steering elements
  in dispersive regions are used.
\item \verb|1| --- Fixed rf frequency, so the beam energy varies. The momentum offset is varied
  using the 4-D orbit finder, iterating until the orbit length equals the design orbit length.
  This mode is robust but can be slow and sometimes converges poorly.
\item \verb|2| --- Full 6-D closed orbit found with a single coupled Newton iteration. It solves
  the transverse orbit, the momentum offset $\delta$, and the rf timing/synchronous phase
  simultaneously, subject to the path-length (fixed-length) constraint, tracking the probe with
  rf time-dependence active. This is generally faster and more robust than \verb|fixed_length=1|.
  It requires an rf cavity that provides longitudinal focusing (i.e. a non-singular $I-R$ in the
  longitudinal plane); with no rf configured the computation is aborted. During this solve the
  \verb|change_t| setting of each rf cavity is internally forced to zero (and restored afterward) so
  that the path-length coordinate is tracked as the absolute orbit length; unlike \verb|fixed_length=1|,
  a nonzero \verb|change_t| is therefore permitted.
\end{itemize}
...
\item \verb|fixed_length| --- If non-zero, indicates that the closed orbit length should be kept the same as the design orbit
  length by changing the momentum offset of the beam. A value of \verb|1| uses the original method, while \verb|2| uses
  a faster 6D method. See \ref{subsec:closedorbit} for more details.
\item \verb|fixed_length_matrix| --- Indicates that for fixed-length orbit correction, the fixed-length
matrix should be computed and used, using either method 1 or 2.
...
\item \verb|reset_correctors_each_step| --- If nonzero (the default), on the first
step after each \verb|run_setup| the steering-corrector values
are snapshotted, and on every subsequent step they are restored to that snapshot
before errors and variations are reapplied.  This ensures that each step starts
from the post-\verb|run_setup| baseline rather than inheriting the previous step's
converged correctors.  Set to 0 to recover the historical behavior in which each
step begins with the prior step's correctors still in place.  Unlike
\verb|prezero_correctors| (which acts only on the initial correction pass within a
single step), this option spans the multi-step loop driven by \verb|n_steps| in
\verb|&run_control|.  The reassertion runs before \verb|do_load_parameters| inside
the steps loop, so \verb|&load_parameters| commands with
\verb|change_defined_values=0| (which take effect inside the loop) still get the
last word: if the parameter file specifies corrector values for the current step,
those values override the reasserted snapshot for that step.
\verb|&load_parameters| with \verb|change_defined_values=1| takes effect when the
command is issued (outside the loop) and is therefore baked into the snapshot
...
matrices.
\item \verb|full_names| --- Flag that, if non-zero, indicates that corrector columns should be named with the element name and
  item name, instead of just the element name.
\item \verb|KnL_units| --- Flag that, if non-zero, indicates use of ``units'' of m/K0L rather than
m/rad.  This results in a sign change for the horizontal data.
\item \verb|BnL_units| --- Flag that, if non-zero, indicates use of ``units'' of m/(T*m) rather than
m/rad.  This is useful for linac work in that the responses are automatically scaled with 
beam momentum.
\item \verb|output_at_each_step| --- Flag that, if non-zero, specifies output of the data at
each simulation step.  By default, the data is output immediately for the defined lattice.
\item \verb|output_before_tune_correction| --- Flag that, if non-zero, specifies that when 
...
\item \verb|reset_correctors_each_step| --- If nonzero (the default), the tune-correcting
  quadrupole strengths are snapshotted on the first \verb|vary_beamline| tick after
  \verb|run_setup| and reasserted at the start of every subsequent step, before
  errors and variations are added.  Set to 0 to inherit the previous step's converged
  tune correctors.  See the description under \verb|&correct| for the wider behavior.
This parameter was added in version 2026.3 and changed the default behavior for multi-step simulations.
\end{itemize}

%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|compute_coupling_response_matrix|}\end{center}
%\end{latexonly}
\subsection{compute\_coupling\_response\_matrix \label{subsec:computecouplingresponsematrix}}

\begin{itemize}
\item type: setup command.
\item function: compute and save the response matrix (derivatives of $\eta_y$ and/or
  cross-plane steering response with respect to correction parameters), for use with
  \verb|correct_coupling|, so that subsequent runs can reuse the same matrix
  via \verb|load_coupling_response_matrix| without rebuilding it.
\item sequence: must follow \verb|run_setup| and \verb|twiss_output|
  so a periodic twiss solution is available when the response is computed.
\item N.B.: can use parallel resources (\verb|Pelegant|). The recommended sequence is to compute the correction
  matrix using parallel resources, then use \verb|load_coupling_response_matrix| when performing coupling
  correction, particularly when post-correction simulation does not require parallel resources.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\begin{itemize}
\item \verb|filename| --- The (incomplete) name of the SDDS file to which the
  $\eta_y$ response matrix is written.  The format includes the BPM/skew element names
  and occurrences plus the matrix entries, so \verb|load_coupling_response_matrix|
  can reconstruct the entire knob/BPM inventory from this single file.
  Suggested value: ``\%s.ccrm''.
\item \verb|cross_filename| --- The (incomplete) name of a {\em second} SDDS
  file to which the cross-plane response matrix is written.  When supplied,
  the cross-plane channel is built alongside $\eta_y$ during this command,
  using \verb|cross_h_steering|, \verb|cross_v_steering|, the cross-plane BPM
  patterns, and \verb|cross_steering_kick| below.  The file records the H/V
  probe-corrector inventory (name, occurrence, item), the cross-plane BPM
  inventory, the response coefficients, and the spec strings themselves as
  parameters so \verb|load_coupling_response_matrix| can reconstruct
  everything without the user re-supplying it.  Suggested value: ``\%s.cprm''.
\item \verb|correction_elements| --- Whitespace/comma-separated list of
  wildcard patterns (one per family) selecting the elements that act as
  correction knobs in the matrix.  Required.  First-match-wins on element
  name; element types lacking the family's \verb|items[k]| parameter are
  silently skipped.  Same semantics as on \verb|correct_coupling|.
\item \verb|items| --- Parallel whitespace-separated list of parameter names,
  one per family.  Defaults to \verb|"K1"| applied to every family.  When the
  family items differ (e.g., \verb|items = "K1 KNL"|) the resulting matrix
  file records the per-knob item in its \verb|KnobParameter| column so
  \verb|load_coupling_response_matrix| reconstructs the per-knob inventory
  correctly.
\item \verb|exclude| --- Optional whitespace-separated list of patterns of
  element names to drop from the knob inventory even if they match one of the
  \verb|correction_elements| entries.
\item \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists
  selecting the beam position monitor (observation) points.  Default
  \verb|bpm_type_pattern="MONI"| picks all two-plane monitor elements.
  Note that the locations don't have to be BPMs, but that makes the most sense.
\item \verb|response_perturbation| --- Finite-difference step in \verb|item|
  used to build each column of $R$ (one-sided difference).  Must be small
  enough to be in the linear regime of the dependence of $\eta_y$ on the
  knob, but large enough to avoid numerical noise.
\item \verb|measurement_noise|, \verb|measurement_noise_cutoff| --- RMS
  amplitude (in meters) and Gaussian cutoff (in units of $\sigma$) of
  simulated independent noise added to every $\eta_y$ reading taken
  while the matrix is being built.  Each of the baseline and the
  $n_{\mathrm{knobs}}$ perturbed readings receives a fresh independent draw,
  matching how a noisy measurement on a real machine accumulates noise
  into the response matrix.  Default 0 produces a noise-free (design) matrix.
  Same semantics as on \verb|correct_coupling|.
\item \verb|cross_h_steering|, \verb|cross_v_steering|,
  \verb|cross_x_bpm_*_pattern|, \verb|cross_y_bpm_*_pattern|,
  \verb|cross_steering_kick|, \verb|cross_measurement_noise| --- Same
  semantics as on \verb|correct_coupling|; see that command's documentation.
  Only consulted when \verb|cross_filename| is supplied; recorded verbatim
  into the cross-plane file so a subsequent \verb|load_coupling_response_matrix|
  carries them through.
\item \verb|verbosity| --- Higher values print more diagnostic information
  while the matrix is being built.
\end{itemize}

The values of \verb|correction_elements| and \verb|items| are recorded
verbatim in the saved file as the \verb|CorrectionElements| and
\verb|CorrectionItems| SDDS parameters (the joined whitespace-separated
family lists), and the resolved per-knob inventory is recorded
explicitly through the element names, occurrences, and the per-knob
\verb|KnobParameter| column.  A subsequent \verb|correct_coupling|
(whether in this run or a later one via \verb|load_coupling_response_matrix|)
can therefore omit \verb|correction_elements| / \verb|items| entirely
and inherit them from the file.  If they {\em are} supplied on
\verb|correct_coupling| they must agree with the values recorded in the
matrix file, or the run is aborted with an explicit diagnostic.  The
\verb|exclude|, \verb|bpm_name_pattern|, and \verb|bpm_type_pattern|
values are recorded implicitly through the resolved BPM and knob
inventories.
A previously-built response matrix can be downsized using the program
\verb|sddsprocess| to reduce the number of correction elements or
observation locations.

%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|compute_lattice_response_matrix|}\end{center}
%\end{latexonly}
\subsection{compute\_lattice\_response\_matrix \label{subsec:computelatticeresponsematrix}}

\begin{itemize}
\item type: setup command.
\item function: compute and save the response matrix
  $R_{ij,k} = \partial o_{i,k})/\partial K_{1,j}$ used by
  \verb|correct_lattice|, where $o_{i,k} \in \{\beta_{i,x}, \beta_{i,y}, \eta_{i,x}\}$
  enumerates the three observables at measurement location $i$.   The saved matrix
  can be reused via \verb|load_lattice_response_matrix| without
  rebuilding it.
\item sequence: must follow \verb|run_setup| and \verb|twiss_output|
  so a periodic twiss solution is available when the response is computed.
\item N.B.: can use parallel resources (\verb|Pelegant|).  The recommended sequence is to compute the response
  matrix using parallel resources, then use \verb|load_lattice_response_matrix| when performing lattice
  correction, particularly when post-correction simulation does not require parallel resources.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\begin{itemize}
\item \verb|filename| --- The (incomplete) name of the SDDS file to which the
  response matrix is written.  The format includes the measuremetn location and knob element names
  and occurrences, the element parameter that was perturbed, plus all three
  response columns (\verb|dBetaxDK1|, \verb|dBetayDK1|, \verb|dEtaxDK1|),
  so \verb|load_lattice_response_matrix| can reconstruct the entire
  location/knob inventory from this single file.  Suggested value: ``\%s.lrm''.
\item \verb|correction_elements| --- Whitespace/comma-separated list of
  wildcard patterns (one per family) selecting the elements that act as
  correction knobs in the matrix.  Required.  Same semantics as on
  \verb|correct_lattice|: first-match-wins on element name; element types
  lacking the family's \verb|items[k]| parameter are silently skipped.
\item \verb|items| --- Parallel whitespace-separated list of parameter names,
  one per family.  Defaults to \verb|"K1"| applied to every family.  When the
  family items differ (e.g., \verb|items = "K1 FSE"|) the resulting matrix
  file records the per-knob item in its \verb|KnobParameter| column so
  \verb|load_lattice_response_matrix| reconstructs the per-knob inventory
  correctly.
\item \verb|exclude| --- Optional whitespace-separated list of patterns of
  element names to drop from the knob inventory even if they match one of
  the \verb|correction_elements| entries.
\item \verb|bind_name_pattern| --- Optional family-binding specification
  recorded with the saved matrix.  Same semantics as on \verb|correct_lattice|
  (each entry collapses every matching knob into one family group; see the
  \verb|correct_lattice| documentation for details).  The intent here is
  to carry the binding alongside the matrix so a subsequent
  \verb|load_lattice_response_matrix| + \verb|correct_lattice| inherits it
  without restating the patterns.  A \verb|correct_lattice| that does
  set its own \verb|bind_name_pattern| overrides what the matrix file
  carries.
\item \verb|measurement_elements|, \verb|measurement_types| --- Parallel
  whitespace/comma-separated wildcard lists specifying the elements at
  which the linear lattice functions are measured.  Both lists must
  have the same token count; an element is selected if for some index
  $i$ its name matches \verb|measurement_elements[i]| {\em and} its
  type matches \verb|measurement_types[i]|.  
  measured via LOCO).  For example, the default values are
  \verb|measurement_elements="* * *"|,
  \verb|measurement_types="MONI HMON VMON"|, which selects all monitor
  elements of any of the three types; whereas
  \verb|measurement_elements="*"|, \verb|measurement_types="*QUAD*"|
  selects every quadrupole element.
\item \verb|response_perturbation| --- Finite-difference step in \verb|item|
  used to build each column of $R$ (one-sided difference).  Must be small
  enough to be in the linear regime of the dependence of $\beta_x$, $\beta_y$,
  and $\eta_x$ on the knob, but large enough to avoid numerical noise.
\item \verb|measurement_noise|, \verb|measurement_noise_cutoff| --- RMS
  amplitude (fractional for $\beta_x$ and $\beta_y$, in meters for 
  $\eta_x$) and Gaussian cutoff (in units of $\sigma$)
  of simulated independent noise added to every observable taken while the
  matrix is being built.  Each of the baseline and the $n_{\mathrm{knobs}}$
  perturbed readings receives a fresh independent draw, matching how a noisy
  measurement on a real machine accumulates noise into the response matrix.
  Default 0 produces a noise-free (design) matrix.
\item \verb|verbosity| --- Higher values print more diagnostic information
  while the matrix is being built.
\end{itemize}
...
family lists), and the resolved per-knob inventory is recorded
explicitly through the element names, occurrences, and the per-knob
\verb|KnobParameter| column.  A subsequent \verb|correct_lattice|
(whether in this run or a later one via \verb|load_lattice_response_matrix|)
can therefore omit \verb|correction_elements| / \verb|items| entirely
and inherit them from the file.  If they {\em are} supplied on
\verb|correct_lattice| they must agree with the values recorded in the
matrix file, or the run is aborted with an explicit diagnostic.  The
\verb|exclude|, \verb|measurement_elements|, and
\verb|measurement_types| values are recorded implicitly through the
resolved measurement-point and knob inventories.

%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|correct_coupling|}\end{center}
%\end{latexonly}
\subsection{correct\_coupling \label{subsec:correctcoupling}}

\begin{itemize}
\item type: setup command.  Execution is deferred to the simulation steps
  loop (like \verb|chromaticity| and \verb|correct_tunes|), where the
  correction is performed once per simulation step between orbit correction
  and tune correction.
\item function: minimize the vertical dispersion $\eta_y$ and/or cross-plane response matrix
  at the selected BPMs by adjusting selected skew-quadrupole (or equivalent) knobs.  
\item sequence: must follow \verb|run_setup| and \verb|twiss_output|.  A
  \verb|track| command (or another action command that triggers the steps
  loop) must follow in order for the correction to actually run.
\item N.B.: can use parallel resources (\verb|Pelegant|) for correction matrix computation.
  However, the recommended sequence is to compute the correction
  matrix using parallel resources with the \verb|compute_coupling_response_matrix|,
  then use \verb|load_coupling_response_matrix| when performing coupling
  correction, particularly when post-correction simulation does not require parallel resources.
  Correction itself will benefit from parallel resources when the cross-plane matrix is included.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\begin{itemize}
\item \verb|correction_elements| --- Whitespace/comma-separated list of
  wildcard patterns (one per family) selecting elements whose parameter will
  be adjusted to correct coupling.  Required unless a prior
  \verb|compute_coupling_response_matrix| or \verb|load_coupling_response_matrix|
  has already populated the knob inventory.  Each element matched by some
  pattern that is not also matched by an \verb|exclude| pattern becomes one
  knob (one SVD column unless joined into a bundle by another command);
  an element matched by multiple patterns is assigned to the first matching
  family (first-match-wins).  Element types that do not expose the family's
  \verb|items[k]| parameter as a double are silently skipped.
\item \verb|items| --- Parallel whitespace-separated list of parameter names,
  one per entry in \verb|correction_elements|.  Defaults to \verb|"K1"|
  applied to every family when omitted.  When supplied the token count must
  match \verb|correction_elements|; otherwise \verb|elegant| bombs with an
  explicit message.
\item \verb|lower_limits|, \verb|upper_limits| --- Parallel space-separated
  signed bounds on the parameter value, one per family.  Either list may be
  omitted to disable that side of the clamp for every knob; when supplied
  the token count must match \verb|correction_elements|.  After every
  iteration the SVD-derived step is uniformly scaled, if necessary, so that
  no knob ends up below its family's \verb|lower_limits| or above its family's
  \verb|upper_limits| value (signed; not magnitudes).  Uniform scaling
  preserves the SVD direction at the cost of a shorter step.
\item \verb|exclude| --- Optional whitespace-separated list of patterns of
  element names to drop from the knob inventory even if they match one of
  the \verb|correction_elements| entries.
\item \verb|bpm_name_pattern|, \verb|bpm_type_pattern| --- Pattern lists for
  the observation BPMs; same semantics as in
  \verb|compute_coupling_response_matrix|.  Ignored when a preloaded matrix
  carries its own BPM list. Note that locations other than BPMs may be specified,
  though this may not make physical sense.
\item \verb|n_iterations| --- Maximum number of correction iterations.
\item \verb|convergence| --- Iteration stops early when the RMS $\eta_y$
  at the BPMs falls below this value (in meters).
\item \verb|change_tolerance| --- Stagnation cutoff on relative
  iteration-to-iteration improvement.  Iteration stops when the
  fractional drop $(r_{k-1} - r_k)/r_{k-1}$ in the weighted RMS residual
  is non-negative but smaller than this value.  Default $0.1$ stops
  when an iteration improved the residual by less than 10\%.  Set to 0
  to disable the check and rely solely on \verb|convergence| /
  \verb|n_iterations|.  A negative improvement (residual grew) is not
  treated as stagnation: the \verb|adaptive_step| machinery at the top
  of the next iteration can roll back and shrink the step instead.
  Must be in $[0, 1)$.
\item \verb|correction_fraction| --- Fraction of the SVD-derived step
  applied at each iteration.  Values $<1$ improve stability by avoiding
  overshoot. Must be in $(0,1]$.
\item \verb|use_perturbed_matrix| --- Controls how often the response
  matrix is (re)computed inside this command:
  \begin{itemize}
  \item 0 (default): build the matrix once during \verb|correct_coupling|
    setup and reuse it for all iterations and all simulation steps.  Cheap;
    accurate when the skew knobs are weak and the optics change very little.
  \item 1: rebuild the matrix at the start of each call to the per-step
    correction routine (once per simulation step).
  \item $>1$: rebuild before each iteration within a call.  Most accurate
    near a coupling resonance, but $\sim n_{\mathrm{iter}} \times n_{\mathrm{skew}}$
    extra twiss recomputations per step.
  \end{itemize}
  If a \verb|compute_coupling_response_matrix| or
  \verb|load_coupling_response_matrix| command has already provided a
  matrix, this control is honored on top of the preloaded matrix.
\item \verb|adaptive_step| --- If nonzero, the effective
  \verb|correction_fraction| is halved whenever an iteration leaves the
  RMS $\eta_y$ larger than at the start of the previous iteration, and the
  skew-knob values are simultaneously rolled back to the previous
  iteration's start.  The next iteration then tries a smaller step from
  the better operating point rather than compounding the bad step.
\item \verb|response_perturbation| --- Finite-difference step used when
  this command builds the matrix itself (only relevant when
  \verb|use_perturbed_matrix>0| or when no preloaded matrix exists).
\item \verb|svd_threshold| --- Singular values below
  \verb|svd_threshold| times the largest singular value are dropped from
  the pseudo-inverse, regularizing the solution.
\item \verb|n_singular_values| --- If $>0$, additionally cap the number of
  retained singular values at this many of the largest.
\item \verb|auto_sv_threshold| --- If nonzero (default), after each iteration's
  step is applied the lattice is checked for stability; if the resulting twiss
  is unstable, the step is rolled back, the working SV-cutoff threshold is
  multiplied by \verb|auto_sv_threshold_factor|, and the SVD is re-solved.
  This is repeated until the lattice is stable, the SV-cutoff drops every
  singular value (in which case no step is applied this iteration), or 20
  retries have elapsed (in which case the step is rolled back and the
  iteration is treated as a no-op).  The working threshold is local to each
  iteration; the next iteration starts again from the namelist
  \verb|svd_threshold|.  Set to 0 to disable the retry loop and accept the
  first-attempt step even when it leaves the lattice unstable.
\item \verb|auto_sv_threshold_factor| --- Multiplicative bump applied to the
  working SV threshold on each retry inside \verb|auto_sv_threshold|.  Default
  3 corresponds to roughly half a decade per retry.
\item \verb|measurement_noise| --- RMS amplitude (in meters) of independent Gaussian
  measurement noise added to every $\eta_y$ reading at every BPM.  Each
  reading -- the baseline measurement, the perturbed measurements used to
  build the response matrix, and the post-correction measurement -- gets a
  fresh, independent draw, matching how a real machine accumulates
  measurement noise into the response matrix and into the residual at the
  noise floor of the BPM system.  Default 0 disables the noise.  
\item \verb|measurement_noise_cutoff| --- Cutoff on the Gaussian noise distribution,
  in units of the standard deviation \verb|measurement_noise|.  Ignored when
  \verb|measurement_noise| is zero.
\item \verb|strength_log| --- The (incomplete) name of an SDDS file
  recording, for each iteration, the parameter changes applied to each
  knob.  Columns include \verb|ElementName|, \verb|ElementOccurence|,
  \verb|s|, \verb|ElementParameter|, \verb|PreviousParameterValue|,
  \verb|DeltaParameterValue|, \verb|ParameterValue|; the parameters include
  \verb|Iteration| and a \verb|Stage| string that is \verb|"uncorrected"|
  for all but the actually-final iteration of the call (which is tagged
  \verb|"corrected"|).
\item \verb|etay_file| --- The (incomplete) name of an SDDS file recording,
  for each iteration, the value of $\eta_y$ at every BPM, with a parameter
  giving the iteration's RMS $\eta_y$.
\item \verb|response_file| --- The (incomplete) name of an SDDS file
  recording the response matrix.  This is in addition to any file written
  by a prior \verb|compute_coupling_response_matrix|.
\item \verb|etay_weight| --- Weight applied to every $\eta_y$ row of the
  residual vector before the weighted SVD solve.  Default 1.0 reproduces
  the original $\eta_y$-only behavior.  Set to 0 to disable the $\eta_y$
  channel entirely (useful when combined with the cross-plane channel).
\item \verb|cross_response_weight| --- Weight applied to every cross-plane
  row of the residual vector.   When both \verb|etay_weight| and
  \verb|cross_response_weight| are positive the two channels are combined
  in a single SVD solve, scaled by their respective weights.
\item \verb|cross_h_steering|, \verb|cross_v_steering| --- Comma-separated
  lists selecting the probe steering correctors used to measure the
  cross-plane orbit response.  Each entry has the form
  \verb|TYPE/ITEM[/NAMEPATTERN]|, where \verb|TYPE| is an element type
  (\verb|EHKICK|, \verb|EVKICK|, \verb|HKICK|, \verb|VKICK|,
  \verb|EKICKER|, \verb|KQUAD|, \verb|DQCOR|, ...), \verb|ITEM| is the
  parameter name of the steering knob on that type (\verb|KICK| for the
  pure correctors; \verb|HKICK|/\verb|VKICK| for combined-function
  elements), and the optional \verb|NAMEPATTERN| (defaulting to \verb|*|)
  restricts the selection to elements whose names match.  Example:
  \verb|"EHKICK/KICK, EKICKER/HKICK, KQUAD/HKICK/*H*"|.  At least one of
  \verb|cross_h_steering| / \verb|cross_v_steering| must be supplied when
  \verb|cross_response_weight > 0|.
\item \verb|cross_x_bpm_name_pattern|, \verb|cross_x_bpm_type_pattern| ---
  Pattern lists selecting BPMs that read the horizontal centroid (used for
  the V-corrector $\to$ x cross-plane block).  Defaults include \verb|MONI|
  and \verb|HMON|.
\item \verb|cross_y_bpm_name_pattern|, \verb|cross_y_bpm_type_pattern| ---
  Pattern lists selecting BPMs that read the vertical centroid (used for
  the H-corrector $\to$ y cross-plane block).  Defaults include \verb|MONI|
  and \verb|VMON|.
\item \verb|cross_steering_kick| --- Magnitude (in radians) of the probe
  kick applied to each steering corrector when measuring the cross-plane
  orbit response.  Must be small enough that the orbit response is in the
  linear regime, but large enough to be well above closed-orbit numerical
  noise.  Default $10^{-5}$.
\item \verb|cross_measurement_noise| --- RMS amplitude (in meters) of
  Gaussian noise added to every cross-plane orbit reading.  Cutoff is
  shared with \verb|measurement_noise_cutoff|.  Default 0 disables the
  noise.
\item \verb|rms_log| --- The (incomplete) name of an SDDS file recording,
  per call to the correction routine, the per-iteration RMS residual
  broken down by channel.  Each page has columns \verb|Iteration|
  (\verb|0| = initial state; \verb|k+1| = state after iteration \verb|k|),
  \verb|EtayRms| (RMS $\eta_y$ at the BPMs, unweighted, in meters),
  \verb|CrossRms| (RMS cross-plane response, m per rad of probe kick;
  \verb|0| when the channel is off), and \verb|WeightedRms| (the actual
  optimization metric driven by the SVD).  Parameters record the channel
  weights in effect.
\item \verb|reset_correctors_each_step| --- If nonzero (the default), the skew-quadrupole
  knob strengths are snapshotted on the first \verb|vary_beamline| tick after
  \verb|run_setup| and reasserted at the start of every subsequent step, before
  errors and variations are added.  Set to 0 to inherit the previous step's converged
  skew strengths.  See the description under \verb|&correct| for the wider behavior.
\item \verb|verbosity| --- Higher values print more diagnostic information.
\end{itemize}
...
%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|correct_lattice|}\end{center}
%\end{latexonly}
\subsection{correct\_lattice \label{subsec:correctlattice}}

\begin{itemize}
\item type: setup command.  Execution is deferred to the simulation steps
  loop (like \verb|chromaticity| and \verb|correct_tunes|), where the
  correction is performed once per simulation step between coupling
  correction and tune correction.
\item function: drive the linear lattice functions $\beta_x$, $\beta_y$,
  and $\eta_x$ at the measurement locations toward design values loaded from a
  user-supplied reference twiss file by adjusting selected normal-quadrupole
  knobs.  This emulates the LOCO-style \cite{Safranek-LOCO} $\beta$-beat and dispersion channels
...
\item sequence: must follow \verb|run_setup| and \verb|twiss_output|.  A
  \verb|track| command (or another action command that triggers the steps
  loop) must follow in order for the correction to actually run.
\item N.B.: can use parallel resources (\verb|Pelegant|) for response matrix computation.
  However, the recommended sequence is to compute the response
  matrix using parallel resources with \verb|compute_lattice_response_matrix|,
  then use \verb|load_lattice_response_matrix| when performing the
  correction, particularly when post-correction simulation does not require parallel resources.
  Correction itself does not benefit from parallel resources since it is based on direct lattice
  function calculations, not simulated LOCO \cite{Safranek-LOCO}.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\item \verb|correction_elements| --- Whitespace/comma-separated list of
  wildcard patterns (one per family) selecting elements whose parameter will
  be adjusted to correct the lattice functions.  Required unless a prior
  \verb|compute_lattice_response_matrix| or \verb|load_lattice_response_matrix|
  has already populated the knob inventory.  Each element matched by some
  pattern that is not also matched by an \verb|exclude| pattern becomes one
  knob (one SVD column unless joined into a bundle by \verb|bind_name_pattern|);
  an element matched by multiple patterns is assigned to the first matching
  family (first-match-wins).  Element types that do not expose the family's
  \verb|items[k]| parameter as a double are silently skipped.
\item \verb|items| --- Parallel whitespace-separated list of parameter names,
  one per entry in \verb|correction_elements|.  Defaults to \verb|"K1"|
  applied to every family when omitted.  When supplied the token count must
  match \verb|correction_elements|.
\item \verb|lower_limits|, \verb|upper_limits| --- Parallel space-separated
  signed bounds on the parameter value, one per family.  Either list may be
  omitted to disable that side of the clamp for every knob; when supplied
  the token count must match \verb|correction_elements|.  After every
  iteration the SVD-derived step is uniformly scaled, if necessary, so that
  no knob ends up below its family's \verb|lower_limits| or above its family's
  \verb|upper_limits| value (signed; not magnitudes).
\item \verb|exclude| --- Optional whitespace-separated list of patterns of
  element names to drop from the knob inventory even if they match one of
  the \verb|correction_elements| entries.
\item \verb|measurement_elements|, \verb|measurement_types| --- Parallel
  wildcard lists selecting the elements at which the linear lattice
  functions are measured; same semantics as in
  \verb|compute_lattice_response_matrix| (see that command's
  documentation for the parallel-list selection rule and examples).
  Ignored when a preloaded matrix carries its own measurement-point
  list.
\item \verb|bind_name_pattern| --- Optional comma- or space-separated
  list of patterns (with \verb|*|/\verb|?| wildcards) defining families
  of knobs whose correction strengths should be {\bf bound} together.
  Each entry of the list defines one family group: every knob whose
  element name matches that entry is placed in a single group and
  receives the same $\Delta K$ from the SVD solver, which is applied
  identically to every member of the group.  Knobs whose names do not
  match any entry remain independent, one SVD column each.  A knob
  matched by multiple entries is placed in the earliest-listed match
  (first match wins).  Example: \verb|bind_name_pattern = "*Q1 *Q2 *Q3"|
  collapses every element ending in \verb|Q1|, every element ending in
  \verb|Q2|, and every element ending in \verb|Q3| into three bound
  family knobs (rather than three groups per distinct name); this is
  the natural way to enforce family symmetry across a periodic ring
  whose elements have per-sector unique names (\verb|S1:Q1|,
  \verb|S2:Q1|, ..., \verb|S40:Q1|).
  Precedence: if a prior \verb|compute_lattice_response_matrix| or
  \verb|load_lattice_response_matrix| already supplied a binding (the
  matrix file records the pattern as a \verb|BindNamePattern| parameter),
  the value on \verb|correct_lattice| {\em overrides} it when given and
  is inherited when omitted (\verb|NULL|).  With no preloaded binding
  and \verb|NULL| here, every knob is independent.
\item \verb|n_iterations| --- Maximum number of correction iterations per
  simulation step.
\item \verb|convergence| --- Iteration stops early when the weighted RMS
  residual at the BPMs falls below this value.
\item \verb|change_tolerance| --- Stagnation cutoff on relative
  iteration-to-iteration improvement.  Iteration stops when the
  fractional drop $(r_{k-1} - r_k)/r_{k-1}$ in the weighted RMS residual
  is non-negative but smaller than this value.  Default $0.1$ stops
...
\item \verb|use_perturbed_matrix| --- Controls how often the response
  matrix is (re)computed inside this command:
  \begin{itemize}
  \item 0 (default): build the matrix once during \verb|correct_lattice|
    setup and reuse it for all iterations and all simulation steps.  Cheap;
    accurate when the knobs are weak and the optics change very little.
  \item 1: rebuild the matrix at the start of each call to the per-step
    correction routine (once per simulation step).
  \item $>1$: rebuild before each iteration within a call.  Most accurate
    far from the design, but $\sim n_{\mathrm{iter}} \times n_{\mathrm{knobs}}$
    extra twiss recomputations per step.
  \end{itemize}
  If a \verb|compute_lattice_response_matrix| or
  \verb|load_lattice_response_matrix| command has already provided a
  matrix, this control is honored on top of the preloaded matrix.
\item \verb|adaptive_step| --- If nonzero, the effective
  \verb|correction_fraction| is halved whenever an iteration leaves the
  weighted RMS residual larger than at the start of the previous iteration,
  and the knob values are simultaneously rolled back to the previous
  iteration's start.  The next iteration then tries a smaller step from
  the better operating point rather than compounding the bad step.
\item \verb|response_perturbation| --- Finite-difference step used when
  this command builds the matrix itself (only relevant when
...
\item \verb|n_singular_values| --- If $>0$, additionally cap the number of
  retained singular values at this many of the largest.
\item \verb|auto_sv_threshold|, \verb|auto_sv_threshold_factor| --- Same
  rollback-and-retry-with-bumped-SV-cutoff behaviour as documented under
  \verb|&correct_coupling|.  Defaults: \verb|auto_sv_threshold = 1|,
  \verb|auto_sv_threshold_factor = 3.0|.
\item \verb|beta_measurement_noise|, \verb|eta_measurement_noise| --- RMS
  amplitude (fractional for beta functions, in meters for dispersion) of independent Gaussian measurement noise added to
  every $\beta_{x,y}$ and $\eta_x$ reading at every BPM, respectively.  Each
  reading---baseline, response-matrix construction, and post-correction
  measurement---gets a fresh, independent draw, matching how a real machine
  accumulates measurement noise into the response matrix and into the
  residual at the noise floor of the BPM system.  Defaults 0 disable the
  noise.
\item \verb|measurement_noise_cutoff| --- Cutoff on the Gaussian noise
  distribution, in units of the standard deviation.  Ignored when both noise
  amplitudes are zero.
\item \verb|reference_file| --- {\bf Required.}  The name of an SDDS file
  (typically a \verb|twiss_output| of the unperturbed design lattice)
  containing the target values of $\beta_x$, $\beta_y$, $\eta_x$ at every
  BPM.  Required columns are \verb|ElementName|, \verb|betax|, \verb|betay|,
  \verb|etax|; the optional column \verb|ElementOccurence| is used when
  present to disambiguate repeated names.  Every BPM matched by the current
  pattern lists must appear in the reference file, or the run is aborted.
\item \verb|betax_weight|, \verb|betay_weight|, \verb|etax_weight| ---
  Per-channel weights applied to the corresponding residual rows (and to
  the corresponding response-matrix rows) before the SVD solve.  This lets
  the user re-balance the channels so that they are comparable in the
  $\ell_2$ sense (for example, weighting $\eta_x$ heavily when the design
  $\eta_x$ is small compared to the design $\beta_x$/$\beta_y$).  All must
  be $>0$.
\item \verb|strength_log| --- The (incomplete) name of an SDDS file
  recording, for each iteration, the parameter changes applied to each
  knob.  Columns include \verb|ElementName|, \verb|ElementOccurence|,
...
\item \verb|response_file| --- The (incomplete) name of an SDDS file
  recording the response matrix.  This is in addition to any file written
  by a prior \verb|compute_lattice_response_matrix|.  Each row corresponds
  to one (BPM, knob) pair with all three observable derivatives
  (\verb|dBetaxDK1|, \verb|dBetayDK1|, \verb|dEtaxDK1|).
\item \verb|rms_log| --- The (incomplete) name of an SDDS file recording
  the per-iteration convergence history.  Each call to the per-step
  correction routine appends a new page; rows within a page are indexed
  by \verb|Iteration|, where 0 is the state before any correction is
  applied and $k+1$ is the state after iteration $k$ has been measured.
  Columns are \verb|Iteration| (long), \verb|BetaxRms|, \verb|BetayRms|,
  \verb|EtaxRms| (each the RMS of the corresponding observable's residual
  $\sqrt{\sum_i (o_i - o_{i,\mathrm{target}})^2 / n_{\mathrm{BPM}}}$, in
  meters, unweighted), and \verb|WeightedRms| (the RMS of the weighted
  residual that the SVD solver actually drives down).  The per-channel
  weights in effect for the page are also recorded as parameters
  \verb|BetaxWeight|, \verb|BetayWeight|, \verb|EtaxWeight|.
\item \verb|reset_correctors_each_step| --- If nonzero (the default), the
  normal-quadrupole knob strengths are snapshotted on the first
  \verb|vary_beamline| tick after \verb|run_setup| and reasserted at the start of
  every subsequent step, before errors and variations are added.  Set to 0 to
  inherit the previous step's converged knob strengths.  See the description
  under \verb|&correct| for the wider behavior.
\item \verb|verbosity| --- Higher values print more diagnostic information.
\end{itemize}
...
\item \verb|calculate_3d_coupling| --- If nonzero (and an rf cavity is present so that the longitudinal
  motion is stable), the eigenvector decomposition is done in full 6-D phase space, producing
  three modes and a meaningful longitudinal-mode contribution to the sigma matrix (bunch length,
  dispersion).  If zero, only the two transverse modes are computed.  Forced to zero when
  \verb|matched=0|, since the longitudinal initial conditions cannot be specified in transport-line
  mode.
\item \verb|verbosity| --- Controls the amount of diagnostic information printed to standard output.
  Higher values produce more detail (eigenvalues, normalized eigenvectors, per-element A and sigma
  matrices).
\item \verb|concat_order| --- Order at which transfer matrices are concatenated when forming the
  one-turn (or transport) map used to find the eigenvectors.

\item \verb|output_sigma_matrix| --- If nonzero, the full upper-triangular sigma matrix is written
  to the output file as columns named \verb|S|$ij$ (1-indexed), in addition to the standard beam-size
  columns.

\item \verb|matched| --- A flag indicating, if non-zero, that the periodic (matched) coupled-twiss
  solution should be found, by diagonalizing the one-turn matrix.  If zero, calculations are
  performed in transport-line mode starting from the user-supplied initial values
  \verb|beta_x1|, \verb|alpha_x1|, etc.\ (or from a \verb|reference_file|).  The longitudinal
  mode is not available in this case, so \verb|calculate_3d_coupling| is forced to zero.

\item \verb|beta_x1|, \verb|beta_x2|, \verb|beta_y1|, \verb|beta_y2| --- If \verb|matched| is zero,
  the contributions of each transverse eigenmode to the corresponding plane $\beta$ functions at
  the start of the beamline.  By convention mode~1 is the $x$-like mode (dominant projection in
  $x$) and mode~2 is the $y$-like mode.  In an uncoupled lattice \verb|beta_x2| and \verb|beta_y1|
  are zero.  These values are the diagonal entries $(A_k)_{xx}$, $(A_k)_{yy}$ of each mode matrix
  $A_k$ at the start.
\item \verb|alpha_x1|, \verb|alpha_x2|, \verb|alpha_y1|, \verb|alpha_y2| --- Initial values of the
  per-mode $\alpha$ functions (sign-flipped off-diagonal entries $-(A_k)_{xx'}$, $-(A_k)_{yy'}$).
\item \verb|gamma_x1|, \verb|gamma_x2|, \verb|gamma_y1|, \verb|gamma_y2| --- Initial values of the
  per-mode $\gamma$ functions ($(A_k)_{x'x'}$, $(A_k)_{y'y'}$).  If left at the default $-1$, the
  code computes $\gamma = (1 + \alpha^2)/\beta$ for the mode's primary plane and
  $\gamma = \alpha^2/\beta$ for the secondary plane.  This rank-2 heuristic is exact for an
  uncoupled initial condition; for a coupled initial condition the user should supply $\gamma$
  explicitly (e.g.\ via \verb|reference_file|), since $\gamma$ is then an independent quantity not
  determined by $(\beta, \alpha)$ alone.
\item \verb|A_xy_1|, \verb|A_xpy_1|, \verb|A_xyp_1|, \verb|A_xpyp_1|,
      \verb|A_xy_2|, \verb|A_xpy_2|, \verb|A_xyp_2|, \verb|A_xpyp_2| ---
  Initial values of the four cross-plane entries of each $A_k$ matrix, i.e.\ the per-mode
  contributions to $\langle x y \rangle$, $\langle x' y \rangle$, $\langle x y' \rangle$,
  $\langle x' y' \rangle$ (each per unit mode emittance).  Default zero (block-diagonal $A_k$).
  Required, along with \verb|gamma_*|, to fully reconstruct a coupled $A_k$ at the start of the
  beamline.
\item \verb|eta_x|, \verb|etap_x|, \verb|eta_y|, \verb|etap_y| --- If \verb|matched| is zero,
  the initial dispersion and dispersion slope, used as the transverse part of the dispersion
  6-vector $\mathbf{d}(0) = (\eta_x, \eta_x', \eta_y, \eta_y', 0, 1)^T$ that is propagated
  through the lattice independently of the transverse mode matrices.

\item \verb|reference_file| --- If given, the name of a previously-produced
  \verb|coupled_twiss_output| file from which the 24 initial-condition values
  (\verb|beta_*|, \verb|alpha_*|, \verb|gamma_*|, the eight \verb|A_*| cross terms, and the four
  \verb|eta*|) are loaded, overriding values supplied in the namelist.  Incompatible with
  \verb|matched=1|.  The file must contain all of the standard \verb|coupled_twiss_output|
  columns (in particular the \verb|gamma*| and \verb|A_*| columns, which were added so that a
  periodic solution can be reproduced exactly by a downstream non-periodic propagation).
\item \verb|reference_element| --- Element in \verb|reference_file| at which to take the initial
  values.  If not given, the values from the last row of \verb|reference_file| are used.
\item \verb|reference_element_occurrence| --- Ignored if \verb|reference_element| is not given.
  Otherwise, the occurrence number of \verb|reference_element| to use.  If 0, the last
  occurrence is used.
\item \verb|reflect_reference_values| --- If nonzero, the loaded values of $\alpha_{x,y}^{(k)}$,
  $\eta_{x,y}'$, and the cross-plane entries \verb|A_xpy_|$k$, \verb|A_xyp_|$k$ are multiplied by
  $-1$ (the entries involving exactly one momentum index).  This corresponds to time-reversal of
  the reference state and permits matching backwards from the reference point.
\end{itemize}
...
\item \verb|name| --- The possibly wildcarded name of the elements for which errors are being specified.
 May alternatively name a knob defined via \verb|&load_knobs| (see \ref{subsec:loadknobs}); in that case
 \verb|item|, \verb|element_type|, \verb|after|, \verb|before|, \verb|fractional|, and \verb|nonadditive|
 are ignored, no wildcards are accepted, and the random draw is always added to the knob's current value,
 which in turn updates every linked element parameter by $\textrm{draw}\cdot\textrm{Factor}$.
...
\item \verb|item| --- The parameter of the elements to which the error pertains.  Required for element
 targets; ignored when \verb|name| is a knob.
...
%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|fast_orbit_feedback|}\end{center}
%\end{latexonly}
\subsection{fast\_orbit\_feedback \label{subsec:fastorbitfeedback}}

\begin{itemize}
\item type: action command.
\item function: simulate fast orbit feedback (FOFB) combined with realistic,
  turn-by-turn beam dynamics.  Unlike \verb|correct|, which computes a static orbit
  correction, this command runs a closed feedback loop: on each iteration it tracks the
  beam for a number of turns, forms a filtered orbit readout at the beam-position
  monitors, computes steering-actuator setpoint changes through an inverse response
  matrix and a per-actuator-class PID controller, and lets the physically applied kicks
  evolve turn-by-turn toward the new setpoints.
\item sequence: must follow \verb|run_setup|; \verb|twiss_output| must precede it (the
  internal response matrix uses the lattice functions, and the RF-frequency actuator uses
  the dispersion and momentum compaction).  A tracking command (e.g. \verb|bunched_beam|)
  provides the beam.
\item can use parallel resources (\verb|Pelegant|); results are identical to the serial
  run for a deterministic beam.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}

The mapping to \verb|run_control| is: each FOFB iteration tracks the beam for
\verb|n_passes| turns (the readout ``tick''), and the number of FOFB iterations is
\verb|n_steps|.  Each turn, every monitor reading is pushed through a first-order digital
IIR filter (see \verb|bpm_filter_file|) so that the tick averages the betatron
oscillation down to the closed orbit.  At the end of each tick the filtered orbit is
projected into actuator space through the internally-built inverse response matrix
($T=-C^{-1}$) and applied through the per-class PID controller; between ticks the
physically applied actuator values evolve turn-by-turn toward their new setpoints through
a per-actuator step-response filter (see the \verb|*_filter_file| options).

By default the actuators are the standard dedicated steering correctors (HKICK/VKICK and
the like) auto-added for each plane.  To choose the actuators explicitly, declare them
with the \verb|steering_element| command using \verb|target="fast_orbit_feedback"|; if any
corrector is declared for a plane, exactly the declared correctors are used for that
plane, otherwise the dedicated correctors are auto-added.

\textbf{RF-frequency actuator.}  Setting \verb|include_rf_frequency=1| adds the RF
frequency as a horizontal actuator (the machine ``radial loop'').  In a ring with
horizontal dispersion, moving the horizontal correctors changes the path length, which
shifts the equilibrium momentum offset $\delta$ and re-distorts the dispersive orbit
$\eta_x\delta$ at the monitors; with the RF frequency held fixed the horizontal plane
therefore shows a slow residual drift that pure steering cannot remove.  The RF frequency
is the DC orbit/energy knob, $\delta=-(1/\alpha_c)(\Delta f/f_0)$, so an integral RF loop
nails zero steady-state horizontal error.  All selected cavities form one joint actuator
(a common fractional detuning) contributing a single column to the horizontal response
matrix, the analytic dispersive column $\partial x_i/\partial f=-\eta_i/(\alpha_c f_0)$,
solved together with the correctors and driven by its own PID class (\verb|rf_Kp/Ki/Kd|,
all zero by default, so the RF loop is off until \verb|rf_Ki| is set; expect
$\verb|rf_Ki|\ll\verb|steering_Ki|$).  Select which cavities are driven with
\verb|steering_element|, \verb|target="fast_orbit_feedback"|, \verb|item="FREQ"| on the
desired \verb|RFCA| elements; if none are declared, every \verb|RFCA| in the beamline is
used.
...
\begin{itemize}
\item \verb|inverse|, \verb|response|, \verb|invert_response| --- Optional files for a
  precomputed inverse (response) matrix, and a flag to invert a supplied response matrix,
  in place of the internally-built one.
\item \verb|keep_largest_SVs|, \verb|remove_smallest_SVs|, \verb|minimum_SV_ratio|,
  \verb|Tikhonov_relative_alpha|, \verb|Tikhonov_n| --- Singular-value-decomposition
  controls for inverting the response matrix, with the same meaning as in \verb|correct|.
\item \verb|steering_Kp|, \verb|steering_Ki|, \verb|steering_Kd| --- Proportional,
  integral, and derivative gains of the PID controller for the steering-corrector
  actuator class.  Pure integral action (\verb|steering_Ki| only) gives bounded
  zero-steady-state-error orbit correction.
\item \verb|rf_Kp|, \verb|rf_Ki|, \verb|rf_Kd| --- PID gains for the RF-frequency actuator
  class (used only when \verb|include_rf_frequency=1|).  All default to zero, so the RF
  loop is off until \verb|rf_Ki| is set.
\item \verb|corrector_limit| --- Maximum allowed absolute value of a steering-corrector
  setpoint (0 = no limit).
\item \verb|rf_frequency_limit| --- Maximum allowed absolute RF-frequency excursion from
  the nominal frequency, in Hz (0 = no limit).
\item \verb|rf_response_scale| --- Internal conditioning scale for the RF column in the
  joint SVD (0 = automatic; the RF column is scaled so it carries weight comparable to the
  corrector columns).  This is a pure conditioning device and does not affect the applied
  physics.
\item \verb|anti_windup| --- If nonzero, integrator anti-windup back-calculation is
  applied when an actuator saturates against its limit.
\item \verb|reset_filters_each_step| --- If nonzero, the BPM and actuator filters are
  reset at the start of each FOFB iteration.
\item \verb|bpm_noise|, \verb|bpm_noise_cutoff|, \verb|bpm_noise_distribution| --- Optional
  monitor-noise amplitude, cutoff, and distribution (``gaussian'' or ``uniform'').
\item \verb|include_rf_frequency| --- If nonzero, enable the RF-frequency actuator (see
  above).  Requires \verb|twiss_output| with dispersion and momentum compaction available.
\item \verb|bpm_filter_file|, \verb|steering_filter_file|, \verb|rf_filter_file| ---
  Optional SDDS files giving the digital-filter (z-transform) coefficients for the monitor
  readout, the steering-corrector step response, and the RF-frequency step response,
  respectively.
\item \verb|output| --- Optional SDDS file recording per-iteration diagnostics (orbit rms
  in each plane, maximum corrector values, pegged-actuator flags, and, when the RF
  actuator is active, the applied RF-frequency detuning \verb|DeltaFrf|).  One SDDS page is
  written and flushed to disk for each feedback iteration as the run proceeds, so the file
  can be examined live while tracking is still in progress rather than only at the end.
\item \verb|output_interval| --- Number of FOFB iterations between records written to
  \verb|output|.
\item \verb|center_on_orbit|, \verb|center_momentum_also| --- If \verb|center_on_orbit| is
  nonzero, the beam is centered on the closed orbit before feedback tracking begins, so it
  starts on the (uncorrected, distorted) orbit rather than on the design orbit; this avoids
  a large betatron transient about the closed orbit at the start of the run.  When
  \verb|center_momentum_also| is nonzero (the default) the longitudinal coordinates are
  centered as well.  This works exactly as in the \verb|track| command, and, as there, it
  takes effect \emph{only} when a \verb|closed_orbit| command precedes the
  \verb|fast_orbit_feedback| command so that the closed orbit is actually computed;
  \verb|fast_orbit_feedback| never computes the closed orbit on its own.  If requested
  without a preceding \verb|closed_orbit| command, a warning is issued and the beam is not
  centered.
\item \verb|offset_by_orbit|, \verb|offset_momentum_also| --- Similar to
  \verb|center_on_orbit|, but the closed-orbit coordinates are \emph{added} to each
  particle's initial coordinates rather than used to shift the beam centroid onto the orbit.
  \verb|offset_momentum_also| (default nonzero) includes the longitudinal coordinates.  The
  same requirement of a preceding \verb|closed_orbit| command applies.
\item \verb|verbosity| --- Larger values produce more diagnostic printout.
\end{itemize}
...
\item \verb|include_vertices| --- Flag that, if non-zero, specifies including  in the output
the coordinates of the vertices of bending magnets. 
\item \verb|vertices_only| --- Flag that, if non-zero, specifies that output will contain only
the coordinates of the vertices of bending magnets. 
\item \verb|magnet_centers| --- Flag that, if non-zero, specifies that output will contain 
...
\item \verb|store_vertices| --- Flag that, if non-zero, results in storing the floor coordinates for
...
%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|load_coupling_response_matrix|}\end{center}
%\end{latexonly}
\subsection{load\_coupling\_response\_matrix \label{subsec:loadcouplingresponsematrix}}

\begin{itemize}
\item type: setup command.
\item function: read a previously-saved coupling-correction response matrix
  (produced by \verb|compute_coupling_response_matrix|) and install it for
  use by a subsequent \verb|correct_coupling| command, avoiding the cost of
...
\begin{itemize}
\item \verb|filename| --- The (incomplete) name of the SDDS file written by
  a prior \verb|compute_coupling_response_matrix| command.  The file
  carries the parameter being adjusted (\verb|ElementParameter|), the
  perturbation that was used to build it, and the full list of BPM and
  skew elements (by name and occurrence); these are matched against the
  current beamline to install the matrix.  An error is reported if any
  expected element is not present in the current beamline.
  The file's \verb|CorrectionElements| and \verb|CorrectionItems|
  parameters (the family-list strings used when the matrix was built)
  are also loaded into the active correction state, so a subsequent
  \verb|correct_coupling| may omit \verb|correction_elements| and
  \verb|items| and inherit them from the matrix file.  When those
  parameters {\em are} restated on \verb|correct_coupling| they must
  agree with the loaded values or the run aborts with an explicit
  diagnostic.
\item \verb|cross_filename| --- Optional.  The (incomplete) name of the
  second SDDS file produced by a prior
  \verb|compute_coupling_response_matrix| run with \verb|cross_filename|
  set.  When supplied, the cross-plane response matrix is also installed:
  the H and V probe-corrector inventories (with their items and
  occurrences), the cross-plane BPM inventories, and the response
  coefficients are restored, and the \verb|cross_h_steering| /
  \verb|cross_v_steering| / probe-kick magnitude from the file become the
  defaults inherited by a subsequent \verb|correct_coupling|.  The user
  still has to opt in by setting \verb|cross_response_weight > 0| on
  \verb|correct_coupling|; the loaded data alone does not auto-enable
  the channel.  Default \verb|NULL| -- no cross-plane channel is
  installed, preserving the original $\eta_y$-only behavior.
\item \verb|verbosity| --- Higher values print more diagnostic information.
\end{itemize}

If either \verb|compute_coupling_response_matrix| or
\verb|load_coupling_response_matrix| is given before
\verb|correct_coupling|, the precomputed matrix is used and the
knob/BPM/item selection on \verb|correct_coupling| is not required.
When the user does restate \verb|correction_elements| or \verb|items|
on \verb|correct_coupling|, the values must match those carried by the
matrix file or the run aborts.  In all cases \verb|lower_limits| and
\verb|upper_limits| may be supplied freely on \verb|correct_coupling|
to clamp the inherited knobs; their token count must agree with the
inherited family count.  If neither command provides a matrix,
\verb|correct_coupling| builds it itself according to its own
\verb|use_perturbed_matrix| control.

%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|load_knobs|}\end{center}
%\end{latexonly}
\subsection{load\_knobs \label{subsec:loadknobs}}

\begin{itemize}
\item type: setup command.
\item function: load \emph{knob} definitions from an SDDS file.  A knob is
        a virtual scalar variable whose value $V$ drives a fixed
        linear combination of element parameters: setting $V$ offsets
        each linked $(\textrm{element},\textrm{parameter})$ pair by
        $(V - V_{\textrm{prev}}) \cdot \textrm{Factor}$, so the lattice
        carries the cumulative contribution of every knob.  Knobs can
        be the target of \verb|&vary_element|,
        \verb|&optimization_variable|, and \verb|&error_element| in
        place of a real element.
\item Command syntax, including use of equations and subcommands, is
        discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
The file must contain one string parameter (\verb|KnobName|) and
three required columns and one optional column:
\begin{itemize}
\item \verb|ElementName| (string) --- the element to drive.  Wildcards
        are supported and expanded at load time using the same
        \verb|wfind_element|/\verb|expand_ranges| idiom as
        \verb|&alter_elements|.
\item \verb|ElementParameter| (string) --- the parameter of that
        element to drive.
\item \verb|Factor| (numeric) --- the per-unit-of-$V$ offset applied
        to that parameter.
\item \verb|ElementOccurence| (long, optional) --- when present, each
        row is resolved to the unique element matching both
        \verb|ElementName| and the given \verb|occurence| value
        (1-based, the same value used in
        \verb|parameter_output|).  This permits the same
        \verb|ElementName| to repeat within a page with different
        occurence values.  Wildcards in \verb|ElementName| are
        \emph{not} permitted when \verb|ElementOccurence| is supplied;
        such rows are rejected at load time.  (Note the elegant
        spelling: ``occurence'', single \emph{r}.)
\end{itemize}
Each page of the file defines one knob.  The page's \verb|KnobName|
parameter is the knob's name and must be unique across all
\verb|&load_knobs| invocations (case-insensitive; names are
upper-cased internally).

Each registered knob is initialized with $V{=}0$ and exposed as the
RPN scalar \verb|<KnobName>.value|, which tracks the knob's current
value across all subsequent \verb|&vary_element|,
\verb|&optimization_variable|, and \verb|&error_element| activity.

\paragraph{Referencing a knob.}
In \verb|&vary_element| / \verb|&optimization_variable| /
\verb|&error_element|, set \verb|name=<KnobName>| (the \verb|item=|
slot is ignored and may be omitted).  The value the command produces
is assigned directly to $V$; the linked element parameters update by
the delta times their respective \verb|Factor| values.  When several
knobs touch the same element parameter, their contributions sum
naturally (each knob's update is delta-from-previous, so the lattice
state at any time is $p = p_0 + \sum_j V_j \cdot F_j$).

\paragraph{Notes and limitations (v1).}
\begin{itemize}
\item \verb|&vary_element| modes \verb|multiplicative| and
        \verb|differential| are rejected for knob targets (the knob's
        intrinsic ``current value'' is the additive scalar $V$).
\item \verb|&error_element| with a knob target is always
        \emph{additive} on $V$; the \verb|fractional|
        and \verb|nonadditive| modifiers do not apply.  The error log
        does not record knob deltas in v1; element parameters
        themselves carry the cumulative effect and can be inspected
        via \verb|parameter_output|.
\item \verb|&save_lattice| does not bake the converged knob value
        into the saved element definitions.  Re-issue
        \verb|&load_knobs| (and re-set the knob values) after
        reloading.
\end{itemize}

\paragraph{Parameters.}
\begin{itemize}
\item \verb|filename| --- {\em Required.}  Path to the SDDS file
        described above.  Subject to the usual
        \verb|compose_filename|/\verb|search_path| resolution.
\item \verb|verbose| --- If nonzero, prints one line per loaded knob
        with the number of \linebreak (element, parameter) targets
        after wildcard expansion.
\end{itemize}

%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|load_lattice_response_matrix|}\end{center}
%\end{latexonly}
\subsection{load\_lattice\_response\_matrix \label{subsec:loadlatticeresponsematrix}}

\begin{itemize}
\item type: setup command.
\item function: read a previously-saved lattice-correction response matrix
  (produced by \verb|compute_lattice_response_matrix|) and install it for
  use by a subsequent \verb|correct_lattice| command, avoiding the cost of
  rebuilding the matrix.
\item sequence: must follow \verb|run_setup|.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\begin{itemize}
\item \verb|filename| --- The (incomplete) name of the SDDS file written by
  a prior \verb|compute_lattice_response_matrix| command.  The file
  carries the parameter being adjusted (\verb|ElementParameter|), the
  perturbation that was used to build it, and the full list of BPM and
  quadrupole elements (by name and occurrence); these are matched against
  the current beamline to install the matrix.  An error is reported if
  any expected element is not present in the current beamline.
  The file's \verb|CorrectionElements| and \verb|CorrectionItems|
  parameters (the family-list strings used when the matrix was built)
  are also loaded into the active correction state, so a subsequent
  \verb|correct_lattice| may omit \verb|correction_elements| and
  \verb|items| and inherit them from the matrix file.  When those
  parameters {\em are} restated on \verb|correct_lattice| they must
  agree with the loaded values or the run aborts with an explicit
  diagnostic.
\item \verb|verbosity| --- Higher values print more diagnostic information.
\end{itemize}

If either \verb|compute_lattice_response_matrix| or
\verb|load_lattice_response_matrix| is given before
\verb|correct_lattice|, the precomputed matrix is used and the
knob/BPM/item selection on \verb|correct_lattice| is not required.
When the user does restate \verb|correction_elements| or \verb|items|
on \verb|correct_lattice|, the values must match those carried by the
matrix file or the run aborts.  In all cases \verb|lower_limits| and
\verb|upper_limits| may be supplied freely on \verb|correct_lattice|
to clamp the inherited knobs; their token count must agree with the
inherited family count.  If neither command provides a matrix,
\verb|correct_lattice| builds it itself according to its own
\verb|use_perturbed_matrix| control.
...
\begin{itemize}
\item type: setup command.
\item function: define parameters for time-dependent (turn-by-turn) modulation of elements.
  In addition to a deterministic amplitude function $A(t)$ from an equation or a shared file column,
  this command can apply an {\em independent per-element} sampled waveform (via
  \verb|element_amplitude_column|) or draw {\em turn-by-turn random noise} from a built-in
  distribution (via \verb|distribution|).  This makes it suitable for modeling mechanical vibration
  (typically the \verb|DX|/\verb|DY| parameters) and strength noise (e.g. the \verb|FSE| parameter),
  applied fresh on every pass during tracking.  Unlike \verb|error_element|, which perturbs the
  lattice once per \verb|run_control| step, \verb|modulate_elements| updates the element on every
  turn, so it can represent a waveform or noise sequence that changes turn-to-turn.
\item sequence: must follow \verb|run_setup|.
\item N.B.: if the ramped element is modeled with a matrix, a significant performance hit may be seen.
  ...
  In particular, on the symplectic kick elements (e.g. \verb|KQUAD|, \verb|KSEXT|, \verb|CSBEND|)
  the \verb|DX|/\verb|DY| offsets and the \verb|FSE| fractional-strength error are read live during
  tracking, so modulating them incurs no per-turn matrix rebuild; the matrix-based elements
  (e.g. \verb|QUAD|, \verb|SBEN|) require a matrix rebuild on every turn.
\item Command syntax, including use of equations and subcommands, is discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
\item \verb|amplitude_column| --- Name of column in \verb|filename| giving amplitude data
  ...
  a function $A(t)$.  This single column is shared by all elements matched by the command, so it
  represents a {\em correlated} waveform (e.g. common ground motion moving a group of elements
  coherently).  Use \verb|element_amplitude_column| instead for independent per-element waveforms.
\item \verb|element_amplitude_column| --- Name of a per-element column {\em template} in
  \verb|filename|, used in place of \verb|amplitude_column| when each matched element is to receive
  its own {\em independent} sampled waveform.  The template is a string containing \verb|%s|, which is
  replaced by the element name, and optionally \verb|%ld|, which is replaced by the occurrence number;
  for example \verb|element_amplitude_column="dy_%s"| loads column \verb|dy_Q1| for element
  \verb|Q1|, column \verb|dy_Q2| for element \verb|Q2|, and so on, while
  \verb|element_amplitude_column="dy_%s_%ld"| would load \verb|dy_Q1_1|, \verb|dy_Q1_2|, etc.  All
  columns share the single \verb|time_column|.  Give exactly one of \verb|amplitude_column| and
  \verb|element_amplitude_column|.
\item \verb|distribution|, \verb|amplitude|, \verb|cutoff|, \verb|correlated| --- Select a built-in
  turn-by-turn random noise source instead of an equation or file.  \verb|distribution| may be
  \verb|"uniform"|, \verb|"gaussian"|, or \verb|"plus_or_minus"| (the same choices, drawn the same
  way, as \verb|error_element|).  \verb|amplitude| gives the RMS (\verb|gaussian|) or half-width
  (\verb|uniform|/\verb|plus_or_minus|), and \verb|cutoff| gives the gaussian truncation in units of
  \verb|amplitude|.  A fresh value is drawn every pass and then combined with the original parameter
  value according to \verb|differential|/\verb|multiplicative|/\verb|factor| below.  If
  \verb|correlated=1| (the default) a single value is drawn per turn and applied to all elements
  matched by the command (they move coherently); if \verb|correlated=0| each matched element gets an
  independent draw each turn.  \verb|correlated| likewise selects between the shared
  (\verb|amplitude_column|) and independent (\verb|element_amplitude_column|) forms of the file source.
  N.B.: the built-in noise is drawn from the same random-number stream that \verb|error_element| uses,
  so it is deterministic and reproducible through \verb|&run_setup random_number_seed|; because the
  two commands share one stream, combining them in the same run makes each dependent on the number of
  draws made by the other.  The stream is seeded identically on all processors, so \verb|elegant| and
  \verb|Pelegant| produce identical results.  Exactly one of \verb|expression|, \verb|filename|, and
  \verb|distribution| may be given.
...
\item \verb|output_at_each_step| --- A flag indicating, if non-zero, that computations and/or output is desired at each step of the simulation.
...
\item \verb|output_before_tune_correction| --- A flag indicating, if non-zero, that output is desired both before and after
tune correction.
\item \verb|final_values_only| --- A flag indicating, if non-zero, that only the final values of the Twiss parameters should
...
\item \verb|matched| --- A flag indicating, if non-zero, that the initial values of the lattice functions should be taken from the
  matched twiss output values.  If zero, then the values given in the command are propagated through the beamline.
\item \verb|radiation| --- A flag indicating, if non-zero, that synchrotron radiation effects should be included.
  N.B.: this flag is all that needs to be set if the lattice contains no kick elements.
  However, if the lattice contains \verb|CSBEND|, \verb|CSRCSBEND|, \verb|KQUAD|, 
  \verb|KSEXT|, or other elements with \verb|SYNCH_RAD| and \verb|ISR| parameters, then the \verb|SYNCH_RAD| and\verb|ISR| must be set to 1 for
  those elements as well.
\item \verb|equilibrium| --- A flag indicating, if non-zero, that the equilibrium moments should be found. 
  Setting this is only of interest for storage rings and usually one wants \verb|radiation=1| at the same time.
\item \verb|force_e1_gt_e2| --- Sometimes the two transverse modes are misidentified, leading to $e_1 < e_2$. If set, this
  flag exchanges the two modes to force $e_1>e_2$.
...
\item \verb|pressure_data| --- The name of an optional SDDS file giving the residual-gas pressure
  profile along the lattice. If provided, the effect of elastic (Coulomb) scattering of the beam
  from residual-gas nuclei is included as a diffusive growth of the transverse divergences. The
  format of this file is described below.
\item \verb|pressure_factor| --- A factor by which all pressure values read from \verb|pressure_data|
  are multiplied as the file is read. This is convenient for scaling an entire pressure profile, for
  example to model a different base pressure or to apply a safety margin, without editing the data
...
If a \verb|pressure_data| file is supplied, \verb|moments_output| includes the emittance growth
due to elastic (Coulomb) scattering of the beam from residual-gas nuclei. At each element a
diffusion term is added to $\langle x^{\prime 2}\rangle$ and $\langle y^{\prime 2}\rangle$ based on the local
partial pressures, the gas species, the gas temperature, and the beam momentum. The same physics is
available during tracking with the \verb|MATTER| element when \verb|FORCE_MCS=1|.

The pressure-data file is an SDDS file with the following structure:
\begin{itemize}
\item Column \verb|s| (units \verb|m|) --- the location along the lattice. The values must be
  monotonically increasing and uniformly spaced (to within 0.1\%). The program \verb|sddsinterp| with
  the \verb|-equispaced| or \verb|-sequence| option can be used to transform data that is not
  equispaced.
\item One floating-point column per gas species, named for that species (see the list below), giving
  the partial pressure of the species as a function of \verb|s|. The units must be \verb|Torr| (or
  equivalently \verb|T|) or \verb|nT| (or equivalently \verb|nTorr|); values given in \verb|nT| are
  converted to \verb|Torr| automatically. Each value is also multiplied by \verb|pressure_factor|.
\item Optional parameter \verb|Gasses| (an SDDS string) --- a comma- or space-separated list of the
  species to use, e.g.\ \verb|"H2O H2 N2 O2 CO2 CO CH4"|. If this parameter is absent, every column
  whose name matches a recognized species is used automatically.
\item Optional parameter \verb|Temperature| (units \verb|K|) --- the gas temperature, used with the
  ideal-gas law to convert pressure to number density. If absent, 293~K is assumed.
\end{itemize}
The recognized gas species are
\verb|H2|, \verb|H2O|, \verb|N2|, \verb|O2|, \verb|CO2|, \verb|CO|, \verb|CH4|, \verb|C|,
\verb|CH3|, \verb|OH|, \verb|C2H2|, \verb|C2H4|, \verb|COH|, \verb|C2H6|, \verb|C2H5|, and
\verb|CF3|. When \verb|Gasses| is absent, columns whose names are not in this list are ignored; if a
species named in \verb|Gasses| is not recognized, a warning is issued.
...
This mode is also available in non-MPI \verb|gpu-elegant|, which processes the
probe particles in bounded batches and prefers CUDA tracking.  If CUDA is
unavailable or an element uses the CPU implementation, the batch may use the
loss-sensitive CPU path; \verb|-ompThreads=N| enables OpenMP parallelism for
supported CPU element loops.  Setting the environment variable
\verb|ELEGANT_GPU_MOMENTUM_BOUNDARY_VERIFY=1| enables an additional,
potentially expensive correctness check: after CUDA tracking, the fixed-grid
boundary points are confirmed with CPU/OpenMP arithmetic and any disagreeing
direction is rescanned on the CPU.  This check is disabled by default.  A
batch containing only one probe uses the CPU
backend without changing from mode 2 to mode 1, since a CUDA launch cannot
amortize its setup cost.  \verb|ELEGANT_GPU_MODE=required| disables this
singleton fallback and continues to require CUDA execution for the primary
ensemble.
...
\item \verb|summarize_setup| --- A flag indicating, if non-zero, that a
...
If a MARK element has {\tt FITPOINT=1} and {\tt COUPLED\_FITPOINT=1}, the
coupled (eigen-)emittances and coupled lattice functions of the tracked beam
(the $\Sigma\cdot J$ / Wolski analysis of the beam matrix, as in {\tt
sddsanalyzebeam -coupled} and the {\tt run\_setup} {\tt coupled\_sigma}
output \cite{Wolski-PRSTAB9-024001}) are made available as additional
symbols.  For each of the three modes $m=1,2,3$ these are
\verb|e|$m$\verb|Beam| and \verb|en|$m$\verb|Beam| (geometric and normalized
eigen-emittance), \verb|betax|$m$\verb|Beam|, \verb|betay|$m$\verb|Beam|,
\verb|betaz|$m$\verb|Beam|, \verb|alphax|$m$\verb|Beam|,
\verb|alphay|$m$\verb|Beam|, \verb|alphaz|$m$\verb|Beam|,
\verb|gammax|$m$\verb|Beam|, \verb|gammay|$m$\verb|Beam|,
\verb|gammaz|$m$\verb|Beam|, and the cross-plane coupling terms
\verb|A_xy_|$m$\verb|Beam|, \verb|A_xpy_|$m$\verb|Beam|,
\verb|A_xyp_|$m$\verb|Beam|, and \verb|A_xpyp_|$m$\verb|Beam|.  The \verb|Beam|
suffix distinguishes these tracked-beam quantities from the like-named
lattice-based coupled Twiss parameters supplied by the \verb|coupled_twiss|
command.
...
\item \verb|soft_failure| --- A flag indicating, if non-zero, that failure of an optimization pass should not
...
the terminal point of the beamline, and up to second order for interior fit points.
...
If \verb|matrix_order=0|, no matrices are computed, which can dramatically speed up optimization for
cases when the matrix must be computed by tracking.
...
\begin{itemize}
\item \verb|method| --- May be one of ``simplex'', ``randomsample'', ``genetic'', ``hybridsimplex'' or ``swarm''. If the
  default ``simplex'' method is chosen, all the processors will do the same optimization as the serial version if there
  is only one particle for optimization tracking, or do optimization tracking in parallel if the number of particles is
  larger than the number of CPUs. All algorithms can be used for global optimization. ``swarm'' is recommended when
  there is sufficient computation resource available, so it can reach the optimization target fast. ``hybridsimplex'' is
  recommended when the initial point is close to the optimal result. ``genetic'' can be chosen for a global optimizer
  with a random start point (0 should be avoided for any initial coordinate). ``randomsample'' may help if other methods
  fail or if it seems advisable to look for solutions far from the starting point.
...
\begin{itemize}
\item \verb|name| --- The name of the element.  May alternatively name a knob defined via
 \verb|&load_knobs| (see \ref{subsec:loadknobs}); in that case \verb|item| is ignored, the optimizer
 treats the knob's scalar value as the variable, and each trial value cascades to every linked
 element parameter as $\Delta\cdot\textrm{Factor}$.  The RPN scalar
 \verb|<KnobName>.value| reflects the current value during optimization.
\item \verb|item| --- The parameter of the element to be varied.  Required for element targets;
 ignored when \verb|name| is a knob.
...
\item \verb|bucket_filename| --- If given, the name of an SDDS file to which the rf-bucket separatrix is
  written in $(t,p)$ coordinates.  The file has columns \verb|t| (time relative to the synchronous particle, in
  seconds), \verb|p| (total momentum $\beta\gamma$ on the separatrix), and \verb|delta| (the fractional momentum
  deviation $(p-p_{\rm central})/p_{\rm central}$).  The scalar parameters \verb|Frequency|, \verb|Harmonic|,
  \verb|Voltage|, \verb|PhiSynch|, and \verb|BucketHalfHeight| are also stored.  One page is written each time the
  rf is set up (i.e., one page per step when \verb|set_for_each_step| is nonzero).  If there is no stable bucket, a zero-length page
  is written.
\item \verb|bucket_points| --- The number of points used for each of the two (upper and lower) branches of the
  separatrix, so that each page contains \verb|2*bucket_points| rows.
\item \verb|name| --- A possibly-wildcard-containing string giving the names of the
...
\item \verb|bunch_frequency|, \verb|step_frequency| --- The frequency to use in calculating the time delay between repetitions.
  These parameters work in similar ways, with an important difference: with \verb|bunch_frequency|, the delay is added to the particle
  coordinates when the beam is generated or loaded. With \verb|step_frequency|, the delay is added to an external register and included
  only in computations related to slow time variation, e.g., cavity mode damping (\verb|RFMODE|, \verb|TRFMODE|, \verb|FRFMODE|, \verb|FTRFMODE|),
  ramping (\verb|RAMPP|, \verb|RAMPRF|), or time-dependent kickers (\verb|BUMPER|). In both cases, the added time delay is $(s-1)/f$, where
  $s=1,...$ is the step and $f$ is the frequency.
\item \verb|n_indices| --- The number of looping indices for which to expect definitions in subsequent
  \verb|vary_element| commands.  If nonzero, then \verb|n_steps| is ignored.
...
\item \verb|coupled_sigma| --- If nonzero, the coupled (eigen-)emittances and coupled
lattice functions of the full 6$\times$6 beam matrix are computed at each element (using the
$\Sigma\cdot J$ eigen-analysis of A. Wolski \cite{Wolski-PRSTAB9-024001}) and appended to the
\verb|sigma| output file.  For each of the three modes $m=1,2,3$ (ordered so that mode~1 is
$x$-dominated, mode~2 $y$-dominated, and mode~3 longitudinal-dominated), the columns
\verb|e|$m$, \verb|en|$m$ (eigen-emittance and normalized eigen-emittance), the per-plane Twiss
functions \verb|betax|$m$, \verb|betay|$m$, \verb|betaz|$m$, \verb|alphax|$m$, \verb|alphay|$m$,
\verb|alphaz|$m$, \verb|gammax|$m$, \verb|gammay|$m$, \verb|gammaz|$m$, and the $x$-$y$ cross-plane
coupling terms \verb|A_xy_|$m$, \verb|A_xpy_|$m$, \verb|A_xyp_|$m$, \verb|A_xpyp_|$m$ are written.
These use the same conventions and column names as the \verb|-coupled| output of
\verb|sddsanalyzebeam|.  The longitudinal plane uses $s$ in meters (see Section
\ref{sec:longitCoord}); default is 0 (off), leaving the file format unchanged.
...
\item \verb|comment| --- If given, this text is written as a comment at the top of the saved file.
Each line is prefixed with the lattice comment character ``\verb|!|'', so the saved file can still be
read back by \verb|elegant|. This is useful for recording provenance, e.g., how or when the lattice
was generated. Multiline comments can be specified by  placing \verb|\n| in the text as an end-of-line indicator.
\end{itemize}
...
\item \verb|save_initial_coordinates| --- A flag that, if non-zero, results in saving initial coordinates
...
\item \verb|target| --- Which command consumes this declaration.  The default,
 \verb|"correct"|, adds the element to the \verb|correct| command's steering lists (the
 historical behavior).  Setting \verb|target="fast_orbit_feedback"| (or \verb|"fofb"|)
 instead routes the declaration to the \verb|fast_orbit_feedback| command's own,
 independent steering lists; see that command for details.  With this target, an
 \verb|item="FREQ"| declaration on an \verb|RFCA| cavity selects that cavity for the
 RF-frequency actuator rather than defining an orbit corrector.
\end{itemize}
...
\item \verb|matched| --- A flag indicating, if non-zero, that the periodic or matched Twiss parameters should be found.
...
\item \verb|output_at_each_step| --- A flag indicating, if non-zero, that output is desired at each step of the simulation.
  If you wish to compute Twiss parameters on a closed orbit or after other calculations, be sure to set this control to a nonzero value.
\item \verb|output_before_tune_correction| --- A flag indicating, if non-zero, that output is desired both before and after
tune correction.
\item \verb|final_values_only| --- A flag indicating, if non-zero, that only the final values of the Twiss parameters should
be output, and not the parameters as a function of s.
\item \verb|statistics| --- A flag indicating, if non-zero, that minimum, maximum, and average values of
Twiss parameters should be computed and included in output.
\item \verb|radiation_integrals| --- A flag indicating, if non-zero, that radiation integrals should be computed
...
When \verb|radiation_integrals=1| is set, the output also includes the
Sokolov-Ternov\cite{SokolovTernov1964} self-polarization time and equilibrium polarization for
a planar ring, computed automatically alongside the existing radiation
integrals.  Expressions in the form
used here are collected in the review by Mane, Shatunov, and
Yokoya~\cite{Mane2005}.  The new parameters are:
\begin{itemize}
\item \verb|I3pos|, \verb|I3neg| --- the sign-resolved third radiation
  integral, $I_{3,\pm} \equiv \int ds/|\rho|^3$ taken separately over the
  positive- and negative-bending regions.  By
  construction \verb|I3pos + I3neg| \verb|= I3|.  Dipoles bin by
  sign of their bend angle; wigglers, undulators and similar symmetric
  insertion devices contribute half to each side since their net
  polarizing contribution is zero.
\item \verb|tauP| --- Sokolov-Ternov self-polarization time
  $\tau_p$ in seconds,
  $\tau_p^{-1} = (5\sqrt{3}/8)\,r_e\hbar\gamma^5 I_3 / (m_e\,C)$,
  with $C$ the ring circumference.
\item \verb|Peq| --- equilibrium polarization (planar-ring reduction
  of the Derbenev-Kondratenko formula),
  $P_{\rm eq} = (8/(5\sqrt{3}))(I_{3,+}-I_{3,-})/I_3$.
  For a planar ring whose dipoles all curve the same way,
  $P_{\rm eq} = \pm P_{\rm ST,limit}$, with the sign indicating
  the direction of polarization relative to the lattice's bend-angle
  convention.  Anti-bends and wigglers reduce
  $|P_{\rm eq}|$ below $P_{\rm ST,limit}$.
\item \verb|PstLimit| --- the ideal Sokolov-Ternov polarization limit
  $8/(5\sqrt{3}) \approx 0.92376$ for a uniform planar guide field.
\end{itemize}
These quantities assume a planar ring with predominantly vertical
guide field; spin-orbit depolarization from horizontal field
components, resonance crossings, and beam-beam effects is not
included.  
...
%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|undulator_brightness|}\end{center}
%\end{latexonly}
\subsection{undulator\_brightness \label{subsec:undulatorbrightness}}

\begin{itemize}
\item type: setup command.
\item function: compute the on-resonance brightness of a planar undulator
        using R. Lindberg's analytic Gaussian-beam formula and expose the
        result as an RPN scalar.  The scalar is refreshed every time the
        Twiss parameters are recomputed --- including each evaluation of
        the optimization function --- so the value tracks lattice changes
        and can be used directly as an optimization target.
\item Command syntax, including use of equations and subcommands, is
        discussed in \ref{sect:generalCommandSyntax}.
\end{itemize}
...
The command may be issued multiple times to register multiple undulator
brightness probes (different photon energies, different undulators,
different observation points in the lattice).  Each registration
creates an RPN scalar named \verb|<tag>.brightness|, distinguished by
the user-supplied \verb|tag| (same naming convention as
\verb|rpn_load|).  Until the first \verb|twiss_output| runs, the
scalar holds the value 0.

The deflection parameter $K$ is {\em not} a namelist input.  It is
derived at every evaluation from the undulator resonance condition
\begin{equation}
\lambda \;=\; \frac{\lambda_u}{2\,n\,\gamma^{2}}\,
              \left(1 + \frac{K^{2}}{2}\right),
\end{equation}
using $\gamma$ obtained from \verb|Pref_output| (which is $\gamma\beta$)
at the chosen Twiss source element, $n$ from \verb|harmonic|,
$\lambda_u$ from \verb|period_length|, and $\lambda$ from either
\verb|wavelength| or \verb|photon_energy|.  If the requested wavelength
is below the fundamental at $K{=}0$, the spec warns once and leaves the
RPN scalar at its previous value.

Beam parameters (the $(\epsilon, \beta, \alpha, \eta, \eta', \sigma_\delta)$
tuple for each plane) are first reduced to their dispersion-corrected
effective values before being passed to the brightness routine, so the
dispersion fields \verb|etax|, \verb|etaxp|, \verb|etay|, \verb|etayp|
are functional inputs, not ornamental.

\begin{itemize}

\item \verb|tag| --- {\em Required.}  Short label prepended to the RPN
        scalar name: the brightness is stored as \verb|<tag>.brightness|.
        Tags must be unique across all \verb|undulator_brightness|
        registrations.

\item \verb|wavelength| --- Photon wavelength in meters.  Exactly one
        of \verb|wavelength| and \verb|photon_energy| must be greater
        than zero.

\item \verb|photon_energy| --- Photon energy in keV.  Internally
        converted to \verb|wavelength| via $\lambda = 12.39841984 \times
        10^{-10}\,\textrm{m}\,/\,E[\textrm{keV}]$.

\item \verb|harmonic| --- Odd harmonic number $n$ at which the brightness
        is evaluated.  Default 1.

\item \verb|detuning| --- Scaled detuning $\Delta\lambda / \lambda =
        (2\pi N/n)\,\textrm{detuning}$ fed through to Lindberg's formula
        as \verb|radDet|.  Good choices are $0$ (on resonance),
        $-0.5$ (default, near the flux maximum), or $-1$ (broader peak).

\item \verb|period_length| --- {\em Required.}  Undulator magnetic
        period $\lambda_u$, meters.

\item \verb|n_periods| --- Number $N$ of magnetic periods.  The total
        undulator length used in the brightness calculation is
        $N\lambda_u$.  Either \verb|n_periods| or \verb|total_length|
        must be supplied (with \verb|total_length| taking precedence
        when both are given).

\item \verb|total_length| --- Optional total length of the undulator,
        meters.  When greater than zero, the period count is derived as
\begin{equation}
N \;=\; \lfloor \, L_{\textrm{tot}} \, / \, \lambda_u \, \rfloor,
\end{equation}
        and the brightness is evaluated for an undulator of length
        $N\lambda_u$ (i.e. truncated to a whole number of periods).
        If \verb|n_periods| is also set to a different value, the
        derived $N$ overrides it and a warning is issued.  If
        \verb|total_length| is less than one \verb|period_length|, the
        run is aborted.

\item \verb|current| --- {\em Required.}  Stored beam current, amperes.

\item \verb|use_twiss_output_values| --- If nonzero, the beam
        emittances, optical functions, and energy spread are taken from
        the live Twiss / radiation-integrals state at the Twiss source
        element described below.  The explicit beam-parameter fields
        (\verb|emitx|, $\dots$, \verb|Sdelta|) are ignored in this mode.
        Requires \verb|coupling > 0| and a prior \verb|&twiss_output|
        invocation with \verb|radiation_integrals = 1|.

\item \verb|coupling| --- {\em Required when} \verb|use_twiss_output_values=1|.
        Transverse coupling parameter $\kappa$ used to split the natural
        emittance computed from radiation integrals:
\begin{equation}
\epsilon_x \;=\; \frac{\epsilon_{x0}}{1 + (J_y/J_x)\,\kappa},
\qquad
\epsilon_y \;=\; \kappa\,\epsilon_x,
\end{equation}
        with damping partition numbers $J_x, J_y$ taken from the
        radiation-integrals structure of the beamline.

\item \verb|twiss_element| --- Optional element name marking the
        location at which the Twiss source values
        ($\beta_{x,y}, \alpha_{x,y}, \eta_{x,y}, \eta'_{x,y}$) and
        $\gamma$ are evaluated.  Defaults to the last element of the
        beamline.  The element need not be a \verb|MARK| --- any
        element with valid Twiss data may be used (in
        \verb|use_twiss_output_values=1| mode, $\epsilon_{x,y}$ and
        $\sigma_\delta$ remain ring-averaged quantities from the
        radiation integrals).

\item \verb|twiss_occurence| --- 1-based occurrence index of
        \verb|twiss_element| in the expanded beamline.  Ignored if
        \verb|twiss_element| is blank.  Default 1. Note that "occurence"
        is deliberately misspelled to be consistent with historical
        usage in the interface.

\item \verb|emitx|, \verb|emity| --- Horizontal and vertical
        geometric emittances, meters$\cdot$radian.  Used only when
        \verb|use_twiss_output_values=0|.  \verb|emitx| must be
        positive; \verb|emity| may be zero (round beam not supported by
        the underlying formula, but values down to the diffraction
        limit are fine).

\item \verb|betax|, \verb|alphax|, \verb|betay|, \verb|alphay| ---
        Twiss parameters at the undulator center, used only when
        \verb|use_twiss_output_values=0|.  \verb|betax| and \verb|betay|
        must be positive.

\item \verb|etax|, \verb|etaxp|, \verb|etay|, \verb|etayp| ---
        Dispersion and its derivative at the undulator center.  Used
        only when \verb|use_twiss_output_values=0|.  These contribute to
        the brightness via the dispersion-corrected effective beam
        parameters; pass zero if dispersion is negligible at the
        observation point.

\item \verb|Sdelta| --- RMS relative energy spread $\sigma_\delta =
        \sigma_E / E$.  Used only when
        \verb|use_twiss_output_values=0|.  Must be positive.

\end{itemize}

The reported brightness is in conventional accelerator units of
...
\item \verb|name| --- The name of an element.  May alternatively name a knob defined via
 \verb|&load_knobs| (see \ref{subsec:loadknobs}); in that case \verb|item| is ignored, the
 \verb|initial|, \verb|final|, and \verb|enumeration_*| values address the knob's scalar value
 directly, and the knob updates every linked element parameter by $\Delta\cdot\textrm{Factor}$
 each vary step.  The \verb|differential| and \verb|multiplicative| modifiers are rejected for
 knob targets (the knob's natural starting value is zero, so additive variation is the only
 sensible mode); \verb|geometric| variation is permitted.
\item \verb|item| --- The parameter of the element to vary.  Required for element targets;
 ignored when \verb|name| is a knob.
...
\item {\tt computeLBGGE} --- Computes generalized gradients from data on a rectangular grid
  for use with the \verb|BGGEXP| element, using the method of D. Sagan {\em et al.} \cite{sagan:ipac23-wepl015}.
\item {\tt computeRBGGE} --- Computes generalized gradients from data on a rectangular boundary 
  for use with the \verb|BGGEXP| element. \verb|computeLBGGE| provides better results and is recommended instead
  when a full three-dimensional field maps is available.
...
%\begin{latexonly}
\newpage
\begin{center}{\Large\verb|computeLBGGE|}\end{center}
%\end{latexonly}
\subsection{computeLBGGE\label{computeLBGGE}}

\begin{itemize}
\item {\bf description:} Compute generalized gradients \cite{Venturini-NIMA427-387} from a three-dimensional field map
  using the local least-squares (``Bmad'') method of D. Sagan {\em et al.} \cite{sagan:ipac23-wepl015}, for use with
  {\tt elegant}'s \verb|BGGEXP| element. Unlike \verb|computeRBGGE|, which reconstructs the generalized gradients from
  field data on four rectangular boundary planes, \verb|computeLBGGE| performs an independent weighted least-squares fit
  at each $z$ plane using field samples drawn from the surrounding volume within a short $z$ window. It is slower than
  \verb|computeRBGGE| but typically provides considerably more accurate fits, and is recommended instead when a full
  three-dimensional field map is available. The output files are in the same format as those from \verb|computeRBGGE|
  and \verb|computeCBGGE|.

\item {\bf synopsis:}
\begin{flushleft}{\tt
computeLBGGE -fieldMap=<filename> -normal=<output> [-skew=<output>]
   [-derivatives=<integer>] [-multipoles=<integer>] [-fundamental=<integer>]
...
\item {\bf switches:}
\begin{itemize}
\item {\tt fieldMap} --- SDDS file containing the three-dimensional field map, with columns \verb|x|, \verb|y|,
  \verb|z| (in meters) and \verb|Bx|, \verb|By|, \verb|Bz| (in Tesla). The distinct \verb|z| values in the map
  define the output $z$ planes and must be uniformly spaced.
\item {\tt normal} ---  Output file for normal-component generalized gradients. Supply to \verb|NORMAL_FILENAME|
  parameter of \verb|BGGEXP|.
\item {\tt skew} ---  Output file for skew-component generalized gradients. Supply to \verb|SKEW_FILENAME|
  parameter of \verb|BGGEXP|. NB: if $B_z$ is non-zero on axis, this option is essential in getting
  valid results.
\item {\tt derivatives} --- Number of derivatives vs z desired in output. Default: 7. With \verb|-autotune|,
  this is the maximum number of derivatives.
\item {\tt multipoles} ---  Number of multipoles desired in output. Default: 8. With \verb|-autotune|,
  this is the maximum number of multipoles.
\item {\tt fundamental} --- Fundamental multipole of sequence. 0=none (default), 1=dipole, 2=quadrupole, etc.
\item {\tt varyDerivatives} --- If given, the number of derivatives used varies with multipole order so as to
  maintain an approximately consistent maximum transverse order.
\item {\tt deltaZ} --- Half-width (in meters) of the $z$ window used for each local fit. By default, twice the
  $z$-plane spacing, giving a five-plane window in the interior.
\item {\tt coreWeight} --- Core weighting factor $w_c\ge 1$; larger values up-weight near-axis samples in the
  fit. Default: 1 (uniform weighting).
\item {\tt xMax}, {\tt yMax} --- Half-aperture of the expansion, in meters. Default: the transverse extent of
  the field map.
\item {\tt xCenter}, {\tt yCenter} --- Center of the expansion, in meters. Default: the midpoint of the field
  map's transverse extent.
\item {\tt threads} --- Number of threads to use for the per-plane fits. Default: 1.
\item {\tt verbose} --- Print progress information.
\item {\tt evaluate} --- Asks to evaluate the GGE and write the reconstructed field, together with the residual
  relative to the field map, at each in-aperture sample point.
\item {\tt autotune} --- Optimizes the number of derivatives and multipoles up to the values given with the
  \verb|-derivatives| and \verb|-multipoles| options, in order to minimize the deviation of the GGE-derived
  fields from the values given in the field map. Because this is a least-squares method, the fit is repeated
  for each candidate model. The three-dimensional map used for the goodness-of-fit test is simply the
  \verb|-fieldMap| already supplied, so no map filename is given here. By default, minimizes the maximum
  deviation, but the user can ask to minimize the rms or mean-absolute-value deviation. Differences below the
  \verb|significance| value are ignored. The \verb|xLimit|, \verb|yLimit|, and \verb|radiusLimit| qualifiers
  restrict the goodness-of-fit test to samples near the axis, measured from the expansion center. The user may
  request verbose output to see results printed to the terminal, and also a log file for a detailed record.
  The \verb|increaseOnly| qualifier specifies that the scan over multipoles $m$ and derivatives $d$ is
  restricted to never be less than the previous optimal values; for example, if the best value so far was
  obtained with $m=4$ and $d=3$, the remainder of the scan would be restricted to $m\ge 4$ and $d\ge 3$; this
  can save considerable run time. The \verb|evaluate| qualifier disables tuning and simply reports the residual
  at the maximum model. The \verb|minDerivatives| and \verb|minMultipoles| parameters allow requiring the
  auto-tuner to use a minimum number of derivatives and multipoles, respectively; by default, as few as 1 of
  each might be used.
\end{itemize}

\item {\bf authors:} Claude Code, M. Borland, R. Soliday, R. Lindberg (ANL/APS); local least-squares method by D. Sagan {\em et al.}
\end{itemize}
...
\item {\bf description:}  Compute generalized gradients \cite{Venturini-NIMA427-387} from data on a rectangular
  boundary \cite{Mitchell-2007} for use with {\tt elegant}'s \verb|BGGEXP| element. When a full three-dimensional
  field map is available, it is better to use \verb|computeLBGGE|, which produces significantly better fits to the field map.
...
[-nowarnings] [-correctedOnly] [-canonical] [-coupled]
}\end{flushleft}
...
\item {\tt -coupled} --- If given, a fully coupled analysis of the beam is performed and additional
  columns giving the coupled (eigen-)emittances and coupled lattice functions are written to the
  output file.  The analysis is a $6\times6$ eigen-analysis of the beam (sigma) matrix itself: the
  eigenvalues of $\Sigma J$ (with $J$ the block-diagonal symplectic form on the
  $(x,x^\prime)$, $(y,y^\prime)$, and $(s,\delta)$ pairs) are $\pm i\,\epsilon_k$, where the three
  invariant eigen-emittances $\epsilon_1,\epsilon_2,\epsilon_3$ reduce to the ordinary $x$, $y$, and
  longitudinal emittances when the planes are uncoupled \cite{Wolski-PRSTAB9-024001}.  The
  longitudinal coordinate is taken as $s = \beta c t$ (m), and the per-mode lattice functions
  ($\beta_{x,k}$, $\alpha_{x,k}$, $\gamma_{x,k}$, the vertical and longitudinal analogs, and the
  $x$--$y$ cross terms) follow the same conventions as {\tt elegant}'s {\tt coupled\_twiss\_output}
  command (Section~\ref{subsec:coupledtwissoutput}), so the results are directly comparable to the
  eigen-emittances $\epsilon_1,\epsilon_2,\epsilon_3$ reported by {\tt moments\_output}
  (Section~\ref{subsec:momentsoutput}).  
\end{itemize}
...
touschekLifetime {\em outputFile}
 ...
 [-ignoreMismatch[=silently]] [-deltaLimit={\em valueInPercent}] [-method={\em 0|1}] 
 [-polarization]
}\end{flushleft}
...
\item {\tt polarization} --- If given, the Touschek lifetime is adjusted to reflect polarization, based on
  \cite{PhysRevAccelBeams.27.124401}.
\end{itemize}
...
\subsection{Highlights of What's New in Version 2026.4}

Here is a summary of what's changed since release 2026.3.
...
\begin{itemize}
\item The \verb|MARK| element will now provide optimizable parameters from coupled-beam eigen-analysis\cite{Wolski-PRSTAB9-024001}
  if \verb|COUPLED_FITPOINT=1|.
\item The \verb|CBSCAT| element was added, providing simulation of Compton backscattering using a method based on \cite{PhysRevAccelBeams.22.040702},
   as well as less approximate methods.
\item The \verb|CHANGE_T| feature of \verb|RFCA| has been enhanced so that subtracted time values are accumulated in extended-precision registers,
  the values from which are used to provide long-scale time offsets (e.g., for slowly-varying, non-harmonic elements) and phase offsets modulo
  $2\pi$ to rf elements. For the latter, this is important if the rf frequency is not a subharmonic of the frequency for which the time value
  changes are done.
\end{itemize}

\subsubsection{New and Modified Commands}
\begin{itemize}
\item The \verb|run_control| command now has a \verb|step_frequency| parameter that can be used to advance the timing of
  some elements independent of the particle time coordinates. This allows slow processes such as ion drift or mode damping to
  occur without introducing rounding-error issues in rf cavities, for example. The need for this was brought to our attention
  by N. Sereno (JLAB).
\item The \verb|sigma| output file from \verb|run_setup| can now optionally include coupled-beam eigen-analysis\cite{Wolski-PRSTAB9-024001}.
  This is invoked by setting \verb|coupled_sigma=1|.
\item Particle counts and particle-loop indices are now stored internally as 64-bit signed integers
  (\verb|int64_t|) throughout tracking, rather than the platform \verb|long| (which is only 32 bits on
  some platforms, such as Windows). This removes the previous $2^{31}$ limit on the number of particles.
  The effective ceiling is now $2^{52}\approx 4.5\times 10^{15}$, which is infinite for all practical purposes.
  This change affects several output files, including the \verb|acceptance|, \verb|centroid|, \verb|final|, \verb|losses|, and \verb|output|, 
  outputs from \verb|run_setup| ; output from \verb|WATCH| elements; \verb|bunch| output from \verb|bunched_beam|; and
  input files to \verb|sdds_beam|.
\item The \verb|save_lattice| command now has a \verb|comment| parameter that can be used to provide a text string that
  will be placed into the saved lattice file as a comment.
\item The \verb|rf_setup| command can now generate a file with the rf separatrix.
\end{itemize}
...
Changes to these tools in this release include
\begin{itemize}
\item The program \verb|computeLBGGE| was added, which implements the local generalized gradient expansion
  method developed by D. Sagan {\em et al.} \cite{sagan:ipac23-wepl015}. It is slower but provides much better fits
  than \verb|computeRBGGE|, although the latter is able to work with boundary data only.
\item The program \verb|sddsanalyzebeam| now optionally computes the eigen-emittances and coupled lattice
  functions for the particle distribution.
\end{itemize}

\subsection{Highlights of What's New in Version 2026.3}
...
\subsubsection{Bug Fixes for Commands}
\begin{itemize}
\item A bug was fixed in IBS calculations for \verb|moments_output| that resulted in non-periodic solutions. The emittances were not
  much affected.
\item A limitation was resolved in coupled twiss parameter computations that resulted in the taking the absolute value of the vertical dispersion.
\end{itemize}

\subsubsection{New and Modified Elements}
\begin{itemize}
\item The \verb|IMPEDANCE| element was added, combining the physics of \verb|ZLONGIT| and \verb|ZTRANSVERSE|. This allows
  combining simulation of short-range monopole, dipole, and quadrupole impedances into a single element, giving a
  significant performance improvement compared to a sequence of separate elements.
\item The \verb|CWAKE| element was added, combining the physics of \verb|WAKE| and \verb|TRWAKE|. This allows combining
  simulation  of short-range monopole, dipole, and quadrupole wakes into a single element, giving a
  significant performance improvement compared to a sequence of separate elements.
\item Spin tracking was added to the \verb|CCBEND| and \verb|LGBEND| elements. With \verb|spin_tracking=1| set
  in \verb|run_setup|, both elements apply Thomas-BMT precession through the body and at the entrance/exit
  edges.
\item For \verb|CCBEND| and \verb|LGBEND|, the body radiation calculation now includes the contribution of
  spurious systematic and random multipoles ({\tt SYSTEMATIC\_MULTIPOLES}, {\tt RANDOM\_MULTIPOLES}, etc.). 
  Previously these were applied to the orbit but omitted from the
  radiation calculation. 
\end{itemize}

\subsubsection{New and Modified Commands}
\begin{itemize}
\item Added the \verb|load_knobs| command, which allows loading knob definitions that link changes in multiple elements to
  a user-defined handle. The handle can be used in \verb|alter_elements|, \verb|error_element|, \verb|optimization_variable|,
  and \verb|vary_element|.
\item Added the \verb|undulator_brightness| command, which computes the x-ray brightness of a specified undulator at a specified
  photon energy, typically for the periodic lattice functions and equilibrium emittance computed by \verb|twiss_output|.
  The results are available for optimization.
\item The \verb|moments_output| command now optionally includes the effect of elastic gas scattering.
\item The \verb|coupled_twiss| command now supplies \verb|alphax1|, \verb|alphax2|, \verb|alphay1|, and \verb|alphay2| in
  the output file, along with other quantities needed for progagation-based calculations.
\item The \verb|coupled_twiss| command now supports propagation of coupled lattice functions by setting \verb|matched=0|.
\item The \verb|correct_coupling|, \verb|compute_coupling_response_matrix|, and \verb|load_coupling_response_matrix| commands
  were added, supplying a method of correcting coupling using the vertical dispersion response to (typically) skew quadrupoles.
\item The \verb|correct_lattice|, \verb|compute_lattice_response_matrix|, and \verb|load_lattice_response_matrix| commands
  were added, supplying a method of correcting the linear lattice functions ($\beta_x$, $\beta_y$, $\eta_x$) at
  selected BPMs by adjusting (typically) normal quadrupoles, against a user-supplied design-twiss reference file.
\item The \verb|correct|, \verb|correct_tunes|, \verb|correct_chromaticity|, \verb|correct_coupling|, and \verb|correct_lattice|
  commands all have a \verb|reset_correctors_each_step| qualifier that can be used to force correctors back to the initial
  values at the start of each step. Previous versions would automatically propagate the previous corrector strengths into the
  next step. This is almost certainly not what is wanted in most cases, in light of which we decided to change the default behavior.
\item The \verb|twiss_output| command now computes the equilibrium polarization and self-polarization time when
  \verb|radiation_integrals=1|.
\item The \verb|parallel_optimization_setup| command now allows selection of the \verb|randomsample| method. If selected, each processor
  uses a different random sequence.
\end{itemize}
...
Changes to these tools in this release include
\begin{itemize}
\item The \verb|touschekLifetime| program now computes the lifetime including polarization, based on
  \cite{PhysRevAccelBeams.27.124401}.
\item The script \verb|swapOutPolarization| was added to model the build-up of polarization during swap-out operation and
  the effect on the beam lifetime.
\end{itemize}

\subsection{Highlights of What's New in Version 2026.2}

Here is a summary of what's changed since release 2026.1.
...
\item The \verb|moments_output| command has the option to force $e_1>e_2$.
\end{itemize}
...
% List references in order of first use (unsrt). Trailing \nocite keeps any
% reference that is not cited in the text, appended after the cited ones.
\nocite{Kernighan,MAD,KLBrown,Borland_thesis,Enge,Borland_PC,Numerical_Recipes,SDDS1,SDDS2,DQS,S2EJitter,TopUpTracking,MingXie,GENESIS,BM,Halbach_69a,Jackson,Ripken,HAPE,Derbenev,Xiao2007SC,Huang2004,Piwinski,Xiao2007a,Warnock,Agoh,Elleaume1992,radia,Bengtsson,ASTRA,IMPACT,TRACK,Chi2005,Zhou-IPAC10,Irwin-PAC95,WangCXDrivingTerms,Bengtsson-SSC232,Bane-SLAC14925,Tsai-RMP46,Wrulich-CAS94,LeDuff-NIM239,sRDT,Floettmann-PRSTAB6-034202,Berenc-IPAC15-MOPMA006,KHwang,TAPAs,Nakamura2004,Chao-PRSTAB-111001,Bacconier-SPS80-2,Venturini-NIMA427-387,GPU1,Bassetti,BuesingPC,RLindbergPC,Sun-NAPAC19,Li-arxiv-1912.00121,Mitchell-2007,Venturini2021,BassettiErskine,Furman-LBL-34682,Borland-LS356,Huang-RCDS,Zhang-PRSTAB-18-010702,HalbachModel,DejusModels,MoogModels,Kim-SCU-Scaling,IvanyushenkovPC,KuboOide,Safranek-LOCO,PhysRevAccelBeams.27.124401,SokolovTernov1964,Mane2005}
