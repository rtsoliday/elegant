...
- Opt-in linear, unsliced, single-bucket `SCMULT`.
...
- Verified the compaction-output change with `ELEGANT_GPU_ENABLE_APERTURE_COMPACTION=1` on `phase3_elimit_loss`; all 5 common SDDS files matched at `1e-11`.  The compaction run reported 143 sync request(s), 143 copy event(s), 142 read-only request(s), and 1 mutable deallocation request.
...
3. Replace the opt-in CSR histogram path with a resident path that avoids per-kick host coordinate copies.
...
- Started on May 6, 2026 with the existing opt-in `ELEGANT_GPU_ENABLE_CSR_HISTOGRAM=1` path, leaving default CSR behavior unchanged.
- Implemented packed-coordinate staging for the CSR histogram slice: the GPU histogram now copies one longitudinal coordinate double per particle instead of staging `totalPropertiesPerParticle` doubles per particle.  CPU-compatible range determination, bin sizing, SG preparation, wake bookkeeping, and particle kicks remain CPU-owned.
...
- Prototyped an opt-in CUDA CSR particle-kick kernel behind `ELEGANT_GPU_ENABLE_CSR_KICK=1`.  The kernel applies the non-filtered CSR `dGamma` interpolation to packed `ct/x/dp` particle data and lets intermediate wake calculations leave `dGamma` resident until host output or final `CSR_LAST_WAKE` state needs it.  Correctness held: `phase6_csr_csbend` and `phase14_csr_saldin54` CPU/GPU quick comparisons matched at `1e-11`, and `phase6_csr_csbend` GPU_VERIFY still passed CSR wake-array and `CSR_LAST_WAKE` checks.
...
- Retested the opt-in CSR kick prototype after the CUDA-host `-O3` rebuild; it did not show a robust speed win at 31.54 seconds for the same 20,000-particle, 261-pass gate and still added CSR kernels plus transfer time, so `ELEGANT_GPU_ENABLE_CSR_KICK=1` remains a correctness-verified stepping stone rather than a recommended speed path.
...
- Added an opt-in resident CSRCSBEND prototype behind `ELEGANT_GPU_ENABLE_CSR_RESIDENT=1`.  For the supported Phase 14 subset (non-backtracking, non-radiating, non-IGF, no wake-filter table, no in-element particle output, no active slice analysis, and no aperture data requiring CPU loss handling), the path copies the already-transformed CSRCSBEND coordinates and `beta0` state to the device, tracks each CSRCSBEND body slice on resident coordinates, computes ct min/max with a dedicated CUDA coordinate-range reduction, fills the histogram directly from device coordinate 4, computes the CSR wake on the device, and applies the CSR kick in place on resident device coordinates.  It still hands particles back to the CPU for final CSRDRIFT handoff state, final edge/coordinate transforms, and unsupported options.
...
- `GPU_VERIFY` quick smoke passed with `ELEGANT_GPU_ENABLE_CSR_RESIDENT=1`: resident `ctHist`, `T1`, `T2`, `dGamma`, and `CSR_LAST_WAKE` checks passed, and final SDDS output matched CPU at `1e-11`.  The existing non-resident `ELEGANT_GPU_ENABLE_CSR_HISTOGRAM=1` quick path also still matched CPU after the resident changes.
...
- Wrapped Phase 14 on May 6, 2026.  The isolated CSR speed gate is met with the opt-in resident CSRCSBEND path: the 20,000-particle, 261-pass `phase6_csr_csbend` workload is 5.27 seconds with CUDA vs 35.24 seconds with CPU, about 6.69x faster, while all 4 common SDDS files match at `1e-11`.  Keep `ELEGANT_GPU_ENABLE_CSR_RESIDENT=1` opt-in for now because the path is intentionally limited to the verified simple CSR subset.

Deferred Phase 14 follow-ups:

- Decide when and how to promote resident CSRCSBEND support from opt-in to default.
...
- Wrapped Phase 15 on May 7, 2026.  The practical aperture/loss gate is met for the verified opt-in subset: focused same-workload `RCOL`, `ECOL`, and ideal `SCRAPER` cases now show useful speedups from about 2.49x to 4.33x while preserving CPU loss semantics at `1e-11`, and the expanded production aperture smoke remains clean across all 70 common SDDS files.  Keep `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=1` opt-in for now because the remaining work is overhead reduction, broader production timing, and unsupported edge-case expansion rather than core correctness.
...
- Turn stable compaction into a recommended production `MAXAMP` loss mode only after it beats the current GPU fallback path by a more comfortable margin across broader same-workload timings.
...
- Decide whether detected-loss fallback in multipole tracking should stay as-is or reuse the Phase 15 stable compaction work for a narrower supported subset.
...
- Add broader deterministic production wrappers from the simple-candidate profile list, such as `latticeErrors6`, `maxamp4`, and `errorSampling*`, before changing default magnet eligibility.
...
5. Keep the current linear single-bucket SCMULT path opt-in until production profiling supports automatic enablement.
...
- SCMULT profile evidence: `scRing2` is the only production test-set case matching the current opt-in linear/unsliced static shape.  `scRing1` and `scRing3` are nonlinear unsliced cases, and `scRing-nonlinearBunched` is nonlinear with bunched-mode input.  This supports measuring `scRing2` before widening the existing opt-in SCMULT path, while keeping nonlinear and bunched SCMULT deferred.
...
- Added `test/gpu_cuda/production_cases/scRing2`, a production wrapper for the current opt-in linear SCMULT CUDA path.  The wrapper uses the source fixed SDDS beam from `/home/soliday/oag/apps/src/elegantTestSet/scRing2`, so bounded benchmarks scale by pass count rather than particle count.
...
- Phase 18 is wrapped for the current CUDA scope as of May 7, 2026.  The implemented slices cover production profiling, the opt-in linear `SCMULT` production case, the narrow zero-length thin-`RFCA` path, bounded field-map/wiggler comparison wrappers, and the cached deterministic `KICKMAP`/`UKICKMAP` prototype, with correctness and timing evidence recorded above.  The remaining Phase 18 items below are deferred follow-ups rather than Phase 18 blockers.  The next active CUDA work should move to Phase 19.

Deferred Phase 18 follow-ups:

- Use the `scRing2` production result to decide whether linear SCMULT should remain opt-in or gain a larger automatic eligibility window.  The current production evidence is correct and 2.56x-2.59x faster, and the thin-RFCA subpath now has a focused regression gate, but automatic SCMULT enablement should still wait for at least one more production-shaped validation case.
...
3. Deferred Phase 18 follow-up: decide whether the now-verified linear `SCMULT` production path should stay opt-in or gain a guarded automatic eligibility path, using `scRing2` plus at least one additional validation shape.
