...
epicsShareFuncOAGPHY extern COMPLEX computeIntegrand(double twoPiSigmapNu, double twoPiDetuneNu,
...
/* Computes effective beam parameters to include the effect of dispersion on
  beam size, divergence, and correlation. 
*/

void computeEffectiveBeamParameters
(
 /* output values */
  double *exEff, double *betaxEff, double *alphaxEff,
 /* input values */
  double ex, double betax, double alphax, double etax, double etaxp, double Sdelta
 )
{
  double S11, S12, S22, gammax;
  S11 = ex*betax + sqr(etax*Sdelta);
  gammax = (1+sqr(alphax))/betax;
  S22 = ex*gammax + sqr(etaxp*Sdelta);
  S12 = -alphax*ex + sqr(Sdelta)*etax*etaxp;
  *exEff = sqrt(S11*S22-sqr(S12));
  *betaxEff = S11/(*exEff);
  *alphaxEff = -S12/(*exEff);
}
