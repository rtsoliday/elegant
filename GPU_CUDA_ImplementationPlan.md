...
- Carry these known Phase 2 limits forward:
  - Historical note: `exactDrift` was originally kept opt-in because early timing did not justify enabling it by default. The finalization pass later superseded this policy and enabled exact drift by default under `ELEGANT_GPU_MIN_EXACT_DRIFT_PARTICLES`, with `ELEGANT_GPU_ENABLE_EXACT_DRIFT=0` as the CPU fallback override.
...
- Historical note: `exactDrift` has a CUDA kernel and a dedicated `exact_drift` benchmark case. It was off by default at the end of the initial Phase 2 pass, but later finalization retesting made it default-on for eligible GPU runs while preserving `ELEGANT_GPU_ENABLE_EXACT_DRIFT=0` as an override.
...
- Historical CUDA exact-drift 60-second baseline with `ELEGANT_GPU_ENABLE_EXACT_DRIFT=1`: 30000 particles, 126 passes, 59.76 seconds. The common 40-pass sample outputs matched all 4 CPU SDDS files. This initial timing note is superseded by the finalization retest that enabled thresholded exact drift by default.
...
- Added opt-in exact CUDA compaction kernels for simple rectangular and elliptical `MAXAMP` losses. These kernels mirrored the CPU swap-with-top compaction order and then synchronized the full particle array back to the host so existing lost-particle bookkeeping remained unchanged. They were disabled by default because current timing was slower than the CPU fallback, and the legacy path was later removed after stable prefix-sum compaction superseded it.
...
- Fresh post-compaction quick SDDS comparisons matched all 5 common files for `phase3_limit_amplitudes`, `phase3_elimit_amplitudes`, default lossy `phase3_limit_loss`, default lossy `phase3_elimit_loss`, and the historical opt-in compacting `phase3_limit_loss`/`phase3_elimit_loss` checks.  The legacy compacting path was later removed after stable prefix-sum compaction superseded it.
...
- Rectangular lossy opt-in CUDA compaction quick timing was CPU 0.15 seconds and GPU 0.30 seconds with CUDA counters `matrix=192`, `reductions=196`, and `apertures=286`; this confirmed correctness but not speed for the legacy path, which was later removed after stable prefix-sum compaction superseded it.
...
- Broader GPU-resident magnet loss handling beyond the default no-loss-output stable compaction subset; `.los` and global loss-coordinate rows still use the checked backup/restore CPU fallback.
...
- Added an experimental fixed-bin CSRCSBEND histogram fill prototype. CPU code still owned range determination, bin sizing, filtering, Savitzky-Golay smoothing, and derivative preparation, while CUDA filled the raw coordinate histogram and `GPU_VERIFY` compared it before the existing wake-array checks. The `gpu-phase6-csr-hist-verify` quick case matched the CPU quick case for all 4 SDDS files at `1e-11`.
- CSR histogram timing did not pass the automatic-enable gate because copying the CPU-tracked coordinates to the GPU for each wake setup cost more than the raw histogram fill saved. The histogram-enabled saved baseline used 20000 particles, 167 passes, and 18.35 seconds; the apples-to-apples 273-pass histogram run took 30.42 seconds and matched the CPU baseline for all 4 SDDS files at `1e-11`, compared with 29.98 seconds for the scratch-only CUDA run. This non-resident prototype was removed after resident CSRCSBEND became the default guarded path.
...
- Added the first Phase 7 CUDA slice for linear, unsliced, single-bucket `SCMULT` kicks. The current slice now keeps the common single-bunch `SCMULT` path GPU-resident by using CUDA beam-sum reductions for rms/centroid calculation, a CUDA bunch-index min/max reduction for first-pass single-bucket discovery, and resident `dmux`/`dmuy` accumulation. CPU code still owns SCMULT insertion, nonlinear kicks, sliced kicks, and multi-bunch cases. The path was introduced as an opt-in and later promoted to default-on for serial CUDA under the same guards, with `ELEGANT_GPU_ENABLE_SCMULT=0` as the fallback override; tune it with `ELEGANT_GPU_MIN_SCMULT_PARTICLES`.
