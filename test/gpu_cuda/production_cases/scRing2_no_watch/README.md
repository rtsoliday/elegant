...
Compare it against `scRing2` with the same pass count and the guarded SCMULT path enabled.  The common physics outputs should continue to match CPU output at tolerance `1e-11`; the `.w1` diagnostic file is intentionally absent.
...
Measured again on May 9, 2026 for action 8: the quick 1000-particle, 8-pass CPU/GPU no-WATCH runs matched all 5 common SDDS files at tolerance `1e-11`, including `.twi`.  CUDA reported 8 resident `SCMULT` kernels, no CPU-element fallback, and only final `gpuBaseDealloc` synchronization.  The quick same-workload speedup was 1.57x; report: `test/gpu_cuda/output/reports/action8-scmult-no-watch-quick.md`.
