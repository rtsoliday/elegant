...
This Phase 18 profile wrapper exercises the only production test-set `SCMULT` case that statically matches the current guarded CUDA shape: linear, unsliced, and single-bucket-like.  It uses the source fixed SDDS beam, so benchmark scaling changes the pass count rather than the particle count.

Quick CPU/GPU/GPU-VERIFY runs matched the common SDDS outputs at tolerance `1e-11`.  The one-minute CPU baseline autoscaled to 601 passes: CPU `elegant` took 58.53s, while the current `gpu-elegant` with the guarded SCMULT path and the Phase 18 thin-RFCA path took 22.86s for the same pass count, a 2.56x throughput speedup.
