...
The CUDA path is intentionally narrow: no radiation, no ISR, no map-element misalignment, and no yaw.  No-loss-output map losses use resident stable compaction by default; set `ELEGANT_GPU_ENABLE_MAGNET_LOSS_COMPACTION=0` to force the older CPU fallback.
