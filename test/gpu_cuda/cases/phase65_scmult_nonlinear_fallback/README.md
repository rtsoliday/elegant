# Phase 65 SCMULT Nonlinear Fallback

Focused action-8 guard for nonlinear `SCMULT` under the guarded SCMULT
policy.  The lattice mirrors the linear
`phase7_scmult_linear` drift ring, but requests `nonlinear=1` so the space
charge kick remains CPU-owned while surrounding supported elements may stay on
the GPU.
