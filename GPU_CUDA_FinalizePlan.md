...
| Basic kernels and residency | Phase 2 kept `exactDrift` opt-in, specialized beam-sum modes on CPU, and host synchronization for output and CPU-only consumers. | Action 11 supersedes the old exact-drift policy: exact drift is now enabled by default for eligible GPU runs, with `ELEGANT_GPU_ENABLE_EXACT_DRIFT=0` as the CPU fallback override. Common reductions are implemented, while specialized reduction modes and MPI reductions still fall back. Output/diagnostics still synchronize when needed. May 11, 2026 `ecol4` showed that diagnostic centroid output can expose near-zero parallel-reduction order residuals, so `centroid`, `bpm_centroid`, and `sigma` output now use CPU reductions by default unless `ELEGANT_GPU_MIN_REDUCTION_PARTICLES` is explicitly set. | Keep exact drift default-on under its particle threshold after the bounded retest; track specialized reductions as low-risk cleanup only after higher-payoff fallbacks are addressed. |
...
- Narrow `removeInvalidParticles` and `imposeApertureData` CUDA paths exist under stable aperture compaction, which is automatic for no-loss-output runs without order-sensitive coordinate output and explicit with `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=1` otherwise.
- Smoothed `WAKE`/`TRWAKE`, `WAKE,CHANGE_P0=1`, tilted single-bunch
  ...
  match-only bunched `WAKE`/`TRWAKE` filtering, nonzero-voltage zero-length thin `RFCA` with
...
- Backtracking runs keep CUDA `LSCDRIFT` and magnet-family tracking on the CPU reference path by default. May 11, 2026 `forwardThenBack1` showed that CUDA LSC plus two magnet kernels left near-zero `Cdelta` centroid differences, while the default CPU-reference backtracking policy passes `byteCompareReferences -v -tolerance 10`.
- Final/coordinate-output runs keep helper-style RF and alignment updates on the CPU reference path by default unless `ELEGANT_GPU_MIN_HELPER_PARTICLES` is explicitly set. May 11, 2026 `frfmode2` showed that default CUDA helper updates left near-zero `ecx`/`ecnx` differences in `run2.fin`; the CPU-reference helper policy passes `byteCompareReferences -v -tolerance 10`.
...
- A narrow nonzero-voltage zero-length thin-`RFCA` path with GPU phase setup, `CHANGE_P0`, and deterministic offsets plus a narrow cached `KICKMAP`/`UKICKMAP` prototype exist. Zero-voltage `RFCA` now follows the CPU no-op/drift path so reference comparisons preserve exact near-zero covariance terms.
...
| --- | --- | --- | --- | --- |
| `ELEGANT_GPU_ENABLE_CSR=0|1`, `ELEGANT_GPU_ENABLE_CSR_RESIDENT=0|1` | CSR tracking is CPU-owned by default; set `ELEGANT_GPU_ENABLE_CSR=1` to exercise CUDA CSR wake kernels, with resident CSR enabled unless `ELEGANT_GPU_ENABLE_CSR_RESIDENT=0` is also set. | The isolated 20,000-particle, 261-pass `phase6_csr_csbend` gate remains the speed target at about 6.69x faster, and earlier focused `GPU_VERIFY` checks passed resident `ctHist`, `T1`, `T2`, and `dGamma`. May 10, 2026 `bunchComp1` isolation showed that default CUDA CSR left near-zero `Cdelta` differences in `run.cen` and `run.san`, while `ELEGANT_GPU_MIN_CSR_PARTICLES=1000000000` passed the reference comparison with matrix/aperture/magnet CUDA still active. | The CSR wake path does not yet reproduce CPU reference output closely enough around zero-energy centroids for precision/relative SDDS checks, especially in CSR output and slice-analysis workflows. | Keep CSR CPU-owned by default. Re-promote with `ELEGANT_GPU_ENABLE_CSR=1` only after a production-shaped CSR reference gate, including `bunchComp1`, passes without near-zero centroid or slice-analysis drift. |
| `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=0|1` | Default-on only for verified no-loss-output aperture loss handling that has no order-sensitive coordinate output; keep `.los`, coordinate `WATCH`, and final particle output on the CPU-order fallback unless the flag is explicitly set to `1`. | Focused same-workload gates show useful `RCOL`, `ECOL`, and `SCRAPER` speedups from about 2.49x to 4.33x when the stable path is forced. The May 8 finalization production smoke matched all 70 common files with the flag enabled; `maxamp1` was 1.83x, while tiny watch-heavy collimator wrappers were still 0.18x-0.22x. A fresh no-loss-output `phase15_elimit_loss_no_output` 30,000-particle, 10-pass run matched CPU at `1e-11` and took 0.85s with automatic stable compaction versus 1.29s CPU and 0.89s with stable compaction explicitly disabled. May 11, 2026 `collimate1` showed that stable compaction preserves survivor `particleID` order while CPU aperture removal uses tail swaps, so unsorted coordinate `WATCH` files differ unless aperture removal follows the CPU path. | `.los` consumers still need row copies to preserve loss output when the stable path is forced, and coordinate output exposes survivor row ordering. The production-style `MAXAMP` margin is modest, and watch-heavy or tiny collimator wrappers remain startup/synchronization dominated. Inverted, all-loss, material-interaction, and broader invalid-particle modes still fall back. | Keep the no-loss-output/no-coordinate-output subset default-on and document `ELEGANT_GPU_ENABLE_APERTURE_PARALLEL_COMPACTION=1` as the explicit stable-order performance override. Do not make `.los` or coordinate-output stable compaction automatic without a design that preserves loss pass, row ordering, global coordinates, and optional spin columns. |
...
| `ELEGANT_GPU_ENABLE_CSBEND=0|1` | Default-off; set `1` to exercise ordinary CUDA `CSBEND` tracking explicitly. | May 11, 2026 `bunchComp6` isolation showed that disabling only magnet kernels made `byteCompareReferences -v -tolerance 100 bunchComp6` pass, while default CUDA `CSBEND` left near-zero `Cdelta` differences in `run.cen`. | The ordinary `CSBEND` CUDA path does not yet reproduce CPU reference longitudinal centroids closely enough around exact-zero values. | Keep ordinary `CSBEND` CPU-owned by default until a production-shaped bending reference gate, including `bunchComp6`, passes without near-zero centroid drift. |
...
| `ELEGANT_GPU_ENABLE_WAKE_TRACKING=0|1` | Default-off; set `1` to exercise the CUDA `WAKE`, `TRWAKE`, and RFCW wake paths explicitly. | May 10, 2026 LCLS1 isolation showed that disabling CUDA wake tracking made `byteCompareReferences -tolerance 10 -v LCLS1` pass while the CUDA wake path left near-zero `run.cen` relative mismatches in `Cx`, `Cxp`, `Cy`, `Cyp`, and `Cdelta`. | The wake binning/reduction and kick path does not yet reproduce the CPU reference closely enough at near-zero centroids for precision/relative SDDS checks. | Keep wake tracking CPU-owned by default. Revisit after CPU-equivalent reductions and production-shaped wake reference gates are in place. |
| `ELEGANT_GPU_ENABLE_RFCA_CHANGE_P0=0|1` | Default-off; set `1` to exercise CUDA RFCA tracking when the element changes `P_central`. | May 10, 2026 `accelBeta3` isolation showed that disabling helper kernels made `byteCompareReferences -tolerance 10 -v accelBeta3` pass, while disabling matrix or aperture kernels did not. The failing columns were near-zero `run.cen` and tiny off-diagonal `run.sig` values after `always_change_p0` RFCA tracking. | The CUDA RFCA path plus CPU reference `do_match_energy` still leaves tiny transverse and sigma roundoff differences around exact zeros. | Keep RFCA `change_p0` CPU-owned by default until the CUDA RFCA update and central-momentum bookkeeping reproduce the CPU reference near zero. |
